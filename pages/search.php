...
if ($k == "" || $internal_share_access)
    {
    # Note current user collection for add/remove links if we have not set it already.
...
if (!$config_search_for_number || !is_numeric($search)) # Don't do this when the search query is numeric, as users typically expect numeric searches to return the resource with that ID and ignore country/date filters.
    ...
    if ($year != "")
        {
        $search = (($search == "")?"":join(", ",split_keywords($search)) . ", ") . "basicyear:" . $year;
        }
    $month = getvalescaped("basicmonth","");
    if ($month != "")
        {
        $search = (($search == "")?"":join(", ",split_keywords($search)) . ", ") . "basicmonth:" . $month;
        }
    $day = getvalescaped("basicday","");
    if ($day != "")
        {
        $search = (($search == "")?"":join(", ",split_keywords($search)) . ", ") . "basicday:" . $day;
        }
    }
...
if (strpos($search,"!") === false)
    {
    rs_setcookie('saved_offset', $offset,0,"","",false,false);
    }
...
rs_setcookie('saved_sort', $sort,0,"","",false,false);
...
if (getvalescaped("resetrestypes","") == "")
    ...
    foreach (array_merge($_GET, $_POST) as $key => $value)
        ...
        if ((substr($key,0,8)=="resource") && !in_array($key, $hiddenfields))
            {
            if ($restypes != "")
                {
                $restypes.=",";
...
if($modified_restypes)
    {
    $restypes = $modified_restypes;
    }
...
if (!array_key_exists("search",$_GET) && !array_key_exists("search",$_POST))
    {
    $offset = getvalescaped("saved_offset",0,true);
    rs_setcookie('saved_offset', $offset,0,"","",false,false);
...
if(($k == "" || $internal_share_access) && strpos($search,"!") === false && $archive_standard)
    {
    $collections = do_collections_search($search,$restypes,0,$order_by,$sort);
...
if ($collectionsearch)
    ...
    if ($k != "" && !$internal_share_access)
        ...
        } # External access - set current collection.
    if (!$collectiondata)
        { ?>
...
// Show featured collection description.
if($collectionsearch)
    { ?>
    ...
    ?> <p><p style="padding:0px"><?php echo $collectiondata['description']?></p> <?php
    }
        ...
        elseif($iconthumbs)
            ...
            if ($xlthumbs==true)
                ...
                <?php if ($display=="xlthumbs")
                    {
                    ?><span class="Selected"><?php echo $lang["xlthumbs"]?></span><?php
                    }
                else
                    {
                    ?><a href="<?php echo generateURL($baseurl_short."pages/search.php",$searchparams,array("display"=>"xlthumbs")) ?>" onClick="return CentralSpaceLoad(this);"><?php echo $lang["xlthumbs"]?></a><?php } ?>&nbsp; |&nbsp;<?php
                    } ?>
            <?php if ($display=="thumbs")
                {
                ?> <span class="Selected"><?php echo $lang["largethumbs"]?></span><?php
                }
            else
                {
                ?><a href="<?php echo generateURL($baseurl_short."pages/search.php",$searchparams,array("display"=>"thumbs")) ?>" onClick="return CentralSpaceLoad(this);"><?php echo $lang["largethumbs"]?></a><?php
                } ?>&nbsp; |&nbsp;
            <?php if ($display=="strip")
                {
                ?><span class="Selected"><?php echo $lang["striptitle"]?></span><?php
                }
            else
                {
                ?><a href="<?php echo generateURL($baseurl_short."pages/search.php",$searchparams,array("display"=>"strip")) ?>" onClick="return CentralSpaceLoad(this);"><?php echo $lang["striptitle"]?></a><?php
                } ?>&nbsp; |&nbsp;
            <?php if ($display=="list")
                {
                ?> <span class="Selected"><?php echo $lang["list"]?></span><?php
                }
            else
                {
                ?><a href="<?php echo generateURL($baseurl_short."pages/search.php",$searchparams,array("display"=>"list")) ?>" onClick="return CentralSpaceLoad(this);"><?php echo $lang["list"]?></a><?php
                } ?>

            <?php hook("adddisplaymode");
