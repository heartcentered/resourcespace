<?php
// Research Functions to Accomodate Research Requests
...
if(!function_exists("send_research_request"))
    ...
    function send_research_request(array $rr_cfields)
        ...
        /** @var string JSON representation of custom research request fields after removing the generated HTML properties
         *  we needed during form processing, @see gen_custom_fields_html_props() */
        ...
        // Emails a resource request (posted) to the team.
        ...
        $templatevars['useremail'] = getvalescaped("email", $useremail); # Use provided email for anonymous access or drop back to user email.
...
function get_research_requests($find = "", $order_by = "name", $sort = "ASC")
    ...
    if($find != "")
        {
        $searchsql = "where name like '%$find%' or description like '%$find%' or contact like '%$find%' or ref='$find'";
        }
    else
        {
        $searchsql = "";
...
if(!function_exists("save_research_request"))
    ...
    function save_research_request($ref)
        ...
        // Check the status, if changed email the originator.
