<?php
// Request Functions to Accomodate Resource Requests and Orders (requests with payment)
...
function save_request($request)
    ...
    // PENDING, moved back to pending. Delete any permissions set by a previous 'approve'.
...
// Request mode 0, emails a collection request (posted) to the team.
function email_collection_request($ref, $details, $external_email)
    ...
    /* Create a copy of the collection which is the one sent to the team. This is so that the admin user can e-mail back an external URL to the collection if necessary, to 'unlock' full (open) access.  The user cannot then gain access to further resources by adding them to their original collection as the shared collection is a copy.  A complicated scenario that is best avoided using 'managed requests'.*/
    $newcopy = create_collection(-1, $lang["requestcollection"]);
    copy_collection($ref, $newcopy);

    // Make sure a collection does not include resources that may have been hidden from the user due to archive state,
    // resource type or access changes and that they are not aware they are requesting.  Without this a full copy can
    // confuse the request administrator.
    ...
    // $userref and $useremail will be that of the internal requestor.  We need to send the $userconfirmmessage to the
    // internal requestor saying that their request has been submitted.
    ...
    // $userref and $useremail will be null for external requestor.  We can only send an email to the email address
    // provided on the external request.
...
// Request mode 1, managed via the administrative interface, an email is still sent.
function managed_collection_request($ref, $details, $ref_is_resource = false)
    ...
    // Has a resource reference (instead of a collection reference) been passed?  Manage requests only work with
    // collections. Create a collection containing only this resource.
    if($ref_is_resource)
        {
        ...
        // Make sure a collection does not include resources that may have been hidden from the user due to archive
        // state, resource type or access changes and that they are not aware they are requesting.  Without this a
        // full copy can confuse the request administrator.
        $col_visible = do_search("!collection" . $ref, '', '', '', -1, 'desc', false, 0, false, false, '', false, false, true);
        $colresources = get_collection_resources($ref);
    ...
    // Hook Processing
    // If assigned admin is absent and default manager (manage_request_admin) is present then request SQL is not
    // established by above hook.  If however, the request SQL has been established, then default manager will be
    // initialised.
    // Regular Processing
    ...
    // Hook Processing
    // If collectiondata is absent then request(s) not created by above hook.  If however. request(s) have been
    // created, then default manager will be initialised.
    // Regular Processing
        ...
        if(isset($collections) && count($collections) > 0)
            ...
            foreach($collections as $request_resource_type => $collection_id)
                ...
                if(array_key_exists($request_resource_type, $manage_request_admin))
                    ...
                    else // We will be sending a notification message.
                        {
    ...
    if($send_default_notifications)
        ...
        if($notify_manage_request_admin)
            ...
            // Do not get default as we may get the requesting user's preference.
            get_config_option($assigned_to_user['ref'], 'email_user_notifications', $send_email, false);
            if($send_email)
                {
                $assigned_to_user = get_user($admin_notify_user);
        ...
        // Send the emails and/or notification messages.
...
// Emails a basic resource request for a single resource (posted) to the team (not a managed request).
