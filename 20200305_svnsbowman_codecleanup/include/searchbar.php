...
else // Pull values from cookies if necessary, for non-search pages where this info has not been submitted.
    {
    if(!isset($restypes))
        {
    ...
    if(trim($keywords[$n]) != "")
        ...
        elseif(strpos($keywords[$n], NODE_TOKEN_PREFIX) !== false)
            ...
            foreach($searched_nodes as $searched_node_index => $searched_node)
                ...
                // Node is not from a simple search field.
        ...
        else // Plain text (non field) search.
            {
            $simple[] = trim($keywords[$n]);
            $initial_tags[] = trim($keywords[$n]);
            }
...
<?php if(!hook("searchbarreplace")) { ?>
    ...
    if(!hook("replacesearchbox") && !$header_search)
        ...
        if($simple_search_pills_view)
            ...
            // Decide when to add tags: If space addTag, if "word" then addTag, do not do anything if open " but not closed.
            jQuery('ul.tag-editor').keydown(function(e)
                ...
                // Find how many double quotes we have in our tag: 1 => spaces are allowed, 2 => add tag.
...
elseif($restypes == '')
    ...
    if($search_includes_user_collections)
        {
        $restypes .= ($restypes == '' ? '' : ',') . "mycol";
        }
    if($search_includes_public_collections)
        {
        $restypes .= ($restypes =='' ? '' : ',') . "pubcol";
        }
    if($search_includes_themes)
        {
        $restypes .= ($restypes =='' ? '' : ',') . "themes";
        }
    ...
    $searchbuttons .= "</div>";
    if(!$searchbar_buttons_at_bottom)
        {
        echo $searchbuttons."<br/>";
        }
    ...
    if(!$basic_simple_search)
        ...
        <?php
        // When using more than one dropdown field, automatically filter field options using AJAX in a attempt to avoid
        // blank results sets through excessive selection of filters.
        if($simple_search_dropdown_filtering && count($optionfields) > 1)
            { ?>
            var Filter="";
            var clickedfieldno="";
            <?php
            $options_fields_count = count($optionfields);
            for($n = 0; $n < $options_fields_count; $n++)
                { ?>
    ...
    function HideInapplicableSimpleSearchFields(reset)
        ...
        for($n = 0; $n < $fields_count; $n++)
            {
            // Check it is not a global field, we do not need to hide those.  Also check it is not a duplicate field as
            // those should not be toggled.
    ...
    <?php if($searchbar_buttons_at_bottom)
        {
        echo $searchbuttons;
        } ?>
...
if ($simple_search_pills_view)
    ...
    jQuery(document).ready(function ()
        {
        // For responsive mode we cannot use tag editor. For some reason it does not work. I think it has something to
        // do with jQuery UI/ layout but not sure what exactly.
