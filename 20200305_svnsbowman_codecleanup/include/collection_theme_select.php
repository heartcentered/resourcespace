<?php
// This file is included from collection_edit.php and collection_set_category.php
...
// Find the number of theme columns.
foreach($collection as $key => $value)
    {
    if (substr($key, 0, 5) == "theme")
        {
...
if(!hook("overridethemesel"))
    ...
    if (checkperm("h") && $enable_themes)
        { ?>
        <input type=hidden name="addlevel" id="addlevel" value=""/> <?php
        ...
        {
        // In case a user can edit collections but does not have themes enabled, preserve them.
        for ($i = 1; $i <= $themecount; $i++)
            {
            if ($theme_category_levels >= $i)
                {
