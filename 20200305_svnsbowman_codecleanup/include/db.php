<?php
/**
 * Database, Data Manipulation, and Generic POST/GET Handling Functions
 *
...
// Functions used for debugging via the System Console.
...
if(isset($remote_config_url) && (isset($_SERVER["HTTP_HOST"]) || getenv("RESOURCESPACE_URL") != ""))
    ...
    if(isset($_SERVER['HTTP_HOST']))
        ...
        {
        // If running scripts from command line the host will not be available and will need to be set as an environment variable, such as export RESOURCESPACE_URL="www.yourresourcespacedomain.com"; cd /var/www/pages/tools; php update_checksums.php
    ...
    if($remote_config !== false && get_sysvar("remote_config-exp" . $hostmd) > time() && !isset($_GET["reload_remote_config"]))
        ...
        {
        // Cache not present or has expired, fetch new config and store. Set a very low timeout of 2 seconds so the config server going down does not take down the site.
        ...
        if($r !== false)
            {
            // Fetch remote config was a success.  Validate the return to make sure it is an expected config file.  The last 33 characters must be a hash and the sign of the previous characters.
            ...
            if($sign == md5($remote_config_key . $r))
                ...
                {
                // Validation of returned config failed. Possibly the remote config server is misconfigured or having issues.  Do nothing, proceed with old configuration and try again later.
                }
            }
        else
            {
            // The attempt to fetch the remote configuration failed.  Do nothing, the cached copy will be used and we will try again later.
            }
...
// End of remote configuration support.
...
function db_use_multiple_connection_modes()
    {
    if(isset($GLOBALS["read_only_db_username"]) && isset($GLOBALS["read_only_db_password"])
...
function db_set_connection_mode($name)
    {
    if(!(is_string($name) && trim($name) !== "") || !db_use_multiple_connection_modes() || !array_key_exists($name, $GLOBALS["db"]))
        {
        return;
        }
...
function db_get_connection_mode()
    {
    if(!db_use_multiple_connection_modes() || !(isset($GLOBALS["db_connection_mode"]) && trim($GLOBALS["db_connection_mode"]) !== ""))
        {
        return "";
        }
...
/**
* Clear the current DB connection mode that is in use to override the current SQL queries. @see db_set_connection_mode() for more details.
*
* @return void
*/
...
/**
* @var  array  Holds database connections for different users (e.g read-write and/or read-only). NULL if no connection has been registered.
*/
$db = null;
function sql_connect()
    {
    global $db, $mysql_server, $mysql_username, $mysql_password, $mysql_db, $mysql_charset, $mysql_force_strict_mode, $mysql_server_port, $use_mysqli_ssl, $mysqli_ssl_server_cert, $mysqli_ssl_ca_cert;
    ...
    foreach($db as $db_connection_mode => $db_connection)
        {
        // Group concat limit increased to support option based metadata with more realistic limit for option entries. Used number of countries (approx 200 * 30 bytes) = 6000 as an example and scaled this up by factor of 5 (arbitrary).
...
if($use_plugins_manager)
    ...
    // Check that manually activated plugins (via config.php) are included in the plugins table.
    foreach($plugins as $plugin_name)
        ...
        if($plugin_name != '')
            ...
            if(sql_value("SELECT inst_version AS value FROM plugins WHERE name='$plugin_name'",'') == '')
                {
                // Installed plugin is not marked as installed in the DB, update it now.  Check if there is a plugin.yaml file to get version and author info.
    ...
    foreach($active_plugins as $plugin)
        {
        // Check group access and YAML, only enable for global access at this point.
    ...
    for($n = $active_plugins_count - 1; $n >= 0; $n--)
        {
...
# Set character set, make sure we are using UTF-8.
...
// Special case for the Team Content Manager, pull in all content from all pages so it is all overridden.
...
// Load the sysvars into an array, useful so we can check migration status, etc.  Note: We should really guard against this by using set_sysvar() and get_sysvar() instead.
...
// Set a system variable that is stored in the sysvars table, set to NULL to remove.
...
// Get a system variable that is stored in the sysvars table.
...
// Plugin architecture, look for hooks with this name and corresponding page, if applicable, and run them sequentially.  Utilises a cache for significantly better performance, enable $draw_performance_footer in config.php to see stats.
        ...
        foreach($hook_cache[$hook_cache_index] as $function)
            ...
            if(!$last_hook_value_wins && !$empty_global_return_value &&
                ...
                if(is_array($function_return_value))
                    ...
                    // Note: in custom plugins developers should work with the full array (ie. superset) rather than just a sub-set of the array. If your plugin needs to know if the array has been modified previously by other plugins use the global variable "hook_return_value".
                    $numeric_key = false;
                ...
                elseif(is_bool($function_return_value))
                    {
                    // Boolean OR.
                    $GLOBALS['hook_return_value'] = $GLOBALS['hook_return_value'] || $function_return_value;
                    }
                }
            else
                {
...
// Used to perform the same DML operation over-and-over-again without the hit of preparing the statement every time.  Useful for re-indexing fields, etc.
// Example usage: sql_query_prepared('INSERT INTO `my_table`(`colint`,`colstring`) VALUES (?,?)',array('is',10,'Ten');
// Where first array parameter indicates types of bind data: i=integer and s=string.
...
// Database functions are wrapped in this way so supporting a database server other than MySQL is
    # easier.  $cache is not used at this time, it was intended for disk based results caching which may be added in the future.  If $fetchrows is set we do not have to loop through all the returned rows.  We just fetch $fetchrows row, but pad the array to the full result set size with empty values. This has been added retroactively to support large result sets, yet a pager can work as if a full result set has been returned as an array (as it was working previously).  $logthis parameter is only relevant if $mysql_log_transactions is set.  0=don't log, 1=always log, 2=detect logging - i.e. SELECT statements will not be logged.
function sql_query($sql, $cache = false, $fetchrows = -1, $dbstruct = true, $logthis = 2, $reconnect = true, $fetch_specific_columns = false)
    ...
    if(db_use_multiple_connection_modes() && (db_get_connection_mode() == "read_only" || ($logthis == 2 && strtoupper(substr(trim($sql), 0, 6)) == "SELECT")))
        {
    ...
    elseif ($result === true) // No result set, query was an INSERT, UPDATE, etc., simply return empty array.
        {
        return $return_rows;
        }
    ...
    while(($fetchrows == -1 || $return_row_count < $fetchrows) && $result_row = mysqli_fetch_assoc($result))
        ...
        if($mysql_verbatim_queries) // No need to do clean up on every cell.
            ...
            else // Dump the specific column into the results set.
                {
                foreach($fetch_specific_columns as $fetch_specific_column)
                    {
                    $return_rows[$return_row_count][$fetch_specific_column] = $result_row[$fetch_specific_column];
                    }
                }
            }
        else
            ...
            else // For specific columns.
                {
                // Iterate through each cell cleaning up.
                foreach($fetch_specific_columns as $fetch_specific_column)
                    {
                    $return_rows[$return_row_count][$fetch_specific_column] = str_replace("\\", "", stripslashes($result_row[$fetch_specific_column]));
                    }
                }
            }
        ++$return_row_count;
    ...
    // If we have not returned all the rows ($fetchrows isn't -1), then we need to fill the array so the count is still correct (even though these rows will not be shown).
...
// Check the database structure against the text files stored in $path and add tables, columns, data, and/or indices as necessary.
function CheckDBStruct($path, $verbose = false)
    ...
    // Load existing tables list first.
    $ts = sql_query("show tables", false, -1, false);
    $ts_count = count($ts);
    $tables = array();

    for($n = 0;$n < $ts_count; $n++)
        {
    ...
    while(($file = readdir($dh)) !== false)
        ...
        if(substr($file, 0, 6) == "table_")
            ...
            if(!in_array($table, $tables))
            ...
            else // Table already exists, so check all columns exist.
                {
                ...
                if($table == "resource")
                    ...
                    for($m = 0; $m < $joins_count; $m++)
                        {
                        // Look for this column in the existing columns.
                        $found = false;

                        for($n = 0; $n < $existing_count; $n++)
                            {
                        ...
                        if(!$found)
                            ...
                            foreach($resources as $resource)
                                ...
                                if($resource_field_data_index !== false
                ...
                // RS-specific mod, add theme columns to collection table as needed.
                        ...
                        if(count($col) > 1)
                            ...
                            for($n = 0; $n < $existing_count; $n++)
                                ...
                                if($existing[$n]["Field"] == $col[0])
                                    ...
                                    // Checks added so that we do not trim off data if a varchar size has been increased manually or by a plugin.
                                    ...
                                    if((count($matchbase) == 3 && count($matchexisting) == 3 && $matchbase[1] == $matchexisting[1] && $matchbase[2] > $matchexisting[2])
            ...
            // Check all indices exist and load existing indexes.
...
// Only escape a string if we need to, to prevent escaping an already escaped string.
...
/**
* Formats a MySQL ISO date.  Always use the 'wordy' style from now on as this works better internationally.
*
...
function redirect($url)
    ...
    else // Base URL has already been added.
        {
...
function setLanguage()
    ...
    if(isset($_GET["language_set"]))
        ...
        if(array_key_exists($language, $languages))
            {
            // Cannot use general.php: rs_setcookie() here since general may not have been included.
            if($global_cookies)
                {
...
function checkperm_user_edit($user)
    {
    // Does not have edit user permission.
    if(!checkperm('u'))
        {
        return false;
        }
    // Allow for passing of user array or user ref to this function.
    if(!is_array($user))
        {
        $user = get_user($user);
        }
    $editusergroup = $user['usergroup'];
    // No user editing restriction, or is not defined so return TRUE.
    if(!checkperm('U') || $editusergroup == '')
        {
        return true;
        }
...
// Update the daily statistics after a loggable event.  The daily_stat table contains a counter for each 'activity type' (i.e. download) for each object (i.e. resource) per day.
if(!function_exists("daily_stat"))
    ...
    function daily_stat($activity_type, $object_ref)
        ...
        // First check to see if there is a row.
...
// For the given plugin shortname, return the path on disk.  Supports plugins being in the filestore folder (for user uploaded plugins).
...
function get_resource_table_joins()
    {
    global $rating_field, $sort_fields, $thumbs_display_fields, $list_display_fields, $data_joins,
    $metadata_template_title_field, $view_title_field, $date_field, $config_sheetlist_fields, $config_sheetthumb_fields, $config_sheetsingle_fields;
...
function debug($text, $resource_log_resource_ref = NULL, $resource_log_code = LOG_CODE_TRANSFORMED)
    ...
    // Output some text to a debug file, for developers only, do not execute if switched off.
...
*
* @return boolean           Success/failure flag, used to prevent certain users from making API calls.
*/
function setup_user($userdata)
    ...
    $ip_restrict_group, $ip_restrict_user, $rs_session, $global_permissions, $userref, $username, $useracceptedterms, $anonymous_user_session_collection, $global_permissions_mask, $user_preferences, $userrequestmode,
    ...
    $userorigin, $actions_enable, $actions_permissions, $actions_on, $usersession, $anonymous_login, $resource_created_by_filter, $user_dl_limit, $user_dl_days, $search_filter_nodes;
    ...
    // This is only required if anonymous user has collection functionality.
    if(isset($anonymous_login) && $username == $anonymous_login && isset($rs_session) && !checkperm('b'))
        {
        ...
        if($anonymous_user_session_collection)
            {
            // Just get the first one if more and hide all links that allow creation of new collections.
            $usercollection = $sessioncollections[0];
            $collection_allow_creation = false;
            }
        else // Unlikely scenario, but maybe we do allow anonymous users to change the selected collection for all other anonymous users.
            {
            $usercollection = $userdata["current_collection"];
            }
        }
    else // Check collection actually exists.
        {
        $usercollection = $userdata["current_collection"];
        ...
        // Old style search filter that has not been migrated.
        if(!$newfilter)
            {
...
/**
* Utility function to remove unwanted HTML tags and attributes. Note: if $html is a full page, developers should allow html and body tags.
*
...
function strip_tags_and_attributes($html, array $tags = array(), array $attributes = array())
    ...
    // Convert to HTML before loading into libxml; otherwise, we will lose non-ASCII characters.
    $html = mb_convert_encoding($html, 'HTML-ENTITIES', 'UTF-8');

    // Basic way of telling whether we had any tags previously.  This allows us to know that the returned value should actually be just text rather than HTML (DOMDocument::saveHTML() returns a text string as a string wrapped in a <p> tag).
    ...
    // Note: this step is required because PHP built-in functions for DOM sometimes do not pick up certain attributes. "Not yet implemented." errors when debugging.
