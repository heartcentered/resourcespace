...
// Ddisplay collection title if option set.
$search_title = "";
$search_title_links = "";
...
if($search_titles_searchcrumbs && $use_refine_searchstring)
    ...
    if($refinements[0] != "")
        ...
        for($n = $startsearchcrumbs; $n < $refinements_count; $n++)
            {
            // Strip the first semi-colon so it is not swapped with an " OR ".
...
if($search_titles)
    ...
    if(substr($search, 0, 11) == "!collection")
        ...
        if($collection_dropdown_user_access_mode)
            ...
            if(!hook('collectionaccessmode'))
                ...
                elseif(strlen($collectiondata["theme"]) == 0)
                    {
    ...
    elseif($search == "" && $archive_standard) // Which resource types (if any) are selected?
        {
        ...
        $resource_types_array_count = count($resource_types_array);
        for($n = 0; $n < $resource_types_array_count; $n++)
            {
        ...
        // All resource types are selected, do not list all of them.
        if(count($searched_resource_types_names_array) == $resource_types_array_count)
            {
        ...
        // All collection types are selected, do not list all of them.
        ...
        $searched_collection_types_names_array_count = count($searched_collection_types_names_array);
        $searched_resource_types_names_array_count = count($searched_resource_types_names_array);

        // Only (one or more) resource types are selected.
        if($searched_resource_types_names_array_count > 0 && $searched_collection_types_names_array_count == 0)
            {
        ...
        elseif($searched_resource_types_names_array_count == 0 && $searched_collection_types_names_array_count > 0)
            {
        ...
        else // No resource types and no collection types are selected, show all resource types and all collection types.
            {
            // Step 1: Replace %resourcetypes%
    ...
    elseif(substr($search, 0, 14) == "!contributions")
        ...
        if($cuser == $userref)
            ...
            if(trim($displayname) == "")
                {
                $displayname = htmlspecialchars($udata["username"]);
                }
    ...
    elseif(!$archive_standard && strpos(",", $archive) === false) // Do not construct title if more than one archive state is selected.
        {
        switch ($archive)
            {
            case -2:
    ...
    elseif(substr($search, 0, 1) != "!" || substr($search, 0, 6) == "!empty")
        {
