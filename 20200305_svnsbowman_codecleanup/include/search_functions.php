<?php
// Search Functions to Perform Read-Only Searches
// For resource indexing and keyword creation, see resource_functions.php.
...
// Returns the most commonly used keyword that sounds like $keyword, or failing a soundex match, the most commonly
// used keyword that starts with the same few letters.
...
// Given an array of resource references ($refs) and the original search query ($search), produce a list of suggested
// search refinements to reduce the result set intelligently.
function suggest_refinement($refs, $search)
    ...
    $refine_count = count($refine);
    for($n = 0; $n < $refine_count; $n++)
        {
...
if(!function_exists("get_advanced_search_fields"))
    ...
    function get_advanced_search_fields($archive = false, $hiddenfields = "")
        ...
        // Date field not present in searchable fields array.
        $date_field_already_present = false;

        // If set, then this is the date field to be added to searchable fields array.
        ...
        if(!$date_field_already_present && $daterange_search && metadata_field_view_access($date_field)
...
// Take the data in the the posted search form that contained $fields, and assemble a search query string that can be
// used for a standard search.  This is used to take the advanced search form and assemble it into a search query.
function search_form_to_search_query($fields, $fromsearchbar = false)
    ...
    for($n = 0; $n < $fields_count; $n++)
            ...
            case FIELD_TYPE_TEXT_BOX_FORMATTED_AND_CKEDITOR:
                ...
                if($value != "")
                    ...
                    for($m = 0; $m < $vs_count; $m++)
                        ...
                        $search .= ((strpos($vs[$m] ,"\"") === false) ? $fields[$n]["name"] . ":" . $vs[$m] : "\"" . $fields[$n]["name"] . ":" . substr($vs[$m], 1, -1) . "\"");
                        }
                    }
                break;
            ...
            case FIELD_TYPE_CHECK_BOX_LIST:
                ...
                if($fields[$n]["display_as_dropdown"])
                    ...
                    if($p != "")
                        ...
                        // This will force each and condition into a separate union in do_search (which will AND).
                        if($checkbox_and)
                            {
                            $p = str_replace(";", ", {$fields[$n]["name"]}:", $p);
                            if($search != "")
                                {
                                $search .= ", ";
                                }
        ...
        // NODES
        // Fixed lists will be handled separately as we do not care about the field they belong to (except when
        // $checkbox_and and $dynamic_keyword_and) we know exactly what we are searching for.
        $node_ref = '';
        ...
        else // Allow a single special search to be prepended to the search string. For example, !contributions<user id>
            {
...
// Function solves several issues related to searching.  It eliminates duplicate terms, helps the field content to
// carry values over into advanced search correctly, fixes a searchbar bug where separators (such as in a pasted
// filename) cause an initial search to fail, separates terms for searchcrumbs.
    ...
    function refine_searchstring($search)
        ...
        foreach($keywords as $keyword)
            ...
            if(strpos($keyword, ":") > 0)
                ...
                elseif(!in_array($keyword, $noadd)) // For searches such as !list.
                    {
...
function compile_search_actions($top_actions)
    ...
    global $baseurl,$baseurl_short, $lang, $k, $search, $restypes, $order_by, $archive, $sort, $daylimit, $home_dash, $url, $allow_smart_collections, $resources_count, $show_searchitemsdiskusage, $offset, $allow_save_search,
    ...
    if(!checkperm('b') && ($k == '' || $internal_share_access))
        ...
        /* Was not able to see this working even in the old code so I left it here for reference. Just uncomment it and it should work.
        global $smartsearch;
...
function search_filter($search, $archive, $restypes, $starsearch, $recent_search_daylimit, $access_override, $return_disk_usage, $editable_only = false)
    {
    global $userref, $userpermissions, $resource_created_by_filter, $uploader_view_override, $edit_access_for_contributor, $additional_archive_states, $heightmin, $heightmax, $widthmin, $widthmax, $filesizemin, $filesizemax, $fileextension, $haspreviewimage, $geo_search_restrict, $pending_review_visible_to_all,
    $search_all_workflow_states, $pending_submission_searchable_to_all, $collections_omit_archived, $k, $collection_allow_not_approved_share, $archive_standard, $open_access_for_contributor, $searchstates;
    ...
    $archive = array_filter($archive, function($state){return (string)(int)$state==(string)$state;});
    ...
    // Geo zone exclusion, a list of upper/lower long/lat bounds, defining areas that will be excluded from geo search
    // results.  Areas are defined as southwest lat, southwest long, northeast lat, northeast long.
    ...
    // Append resource type restrictions based on 'T' permission, look for all 'T' permissions and append to the SQL filter.
    $rtfilter = array();
    $userpermissions_count = count($userpermissions);
    for($n = 0; $n < $userpermissions_count; $n++)
        {
    ...
    // Append standard archive searching criteria. Updated 01/2016 to apply to collections as resources in a pending state that are in a shared collection could bypass approval process.
    if(!$access_override)
        {
        if(substr($search, 0, 11) == "!collection" || substr($search, 0, 5) == "!list" || substr($search, 0, 15) == "!archivepending" || substr($search, 0, 12) == "!userpending")
            {
            // Resources in a collection or list may be in any archive state, other special searches define the
            // archive state in search_special().
        ...
        if(!checkperm("v") && !(substr($search, 0, 11) == "!collection" && $k != '' && $collection_allow_not_approved_share))
            ...
            if(!$pending_submission_searchable_to_all)
                {
                $pending_states_visible_to_all_sql .= "(r.archive<>-2 OR r.created_by='" . $userref . "')";
                }

            if(!$pending_review_visible_to_all)
                {
                $pending_states_visible_to_all_sql .= (($pending_states_visible_to_all_sql != "") ? " AND " : "") . "(r.archive<>-1 OR r.created_by='" . $userref . "')";}
            ...
            if($pending_states_visible_to_all_sql != "")
                ...
                for($n = 0; $n < $userpermissions_count; $n++)
                    ...
                    if(substr($userpermissions[$n], 0, 3) == "ert")
                        ...
                        if(is_numeric($rt))
                            {
                            $rtexclusions .= " OR (resource_type=" . $rt . ")";
                            }
                        }
                    }
    ...
    if($editable_only)
        ...
        if(!checkperm("v") && !$access_override)
            {
            // Condition added 2020-03-02 so that resources without an entry in the resource_custom_access table are included in the search results - "OR (rca.access IS NULL AND rca2.access IS NULL)"
        ...
        // Construct resource type exclusion based on 'ert' permission, look for all 'ert' permissions and append to the exclusion array.
        $rtexclusions = array();
        $userpermissions_count = count($userpermissions);
        for($n = 0; $n < $userpermissions_count; $n++)
            {
...
function search_special($search, $sql_join, $fetchrows, $sql_prefix, $sql_suffix, $order_by, $orig_order, $select, $sql_filter, $archive, $return_disk_usage, $return_refs_only = false, $returnsql = false)
    ...
    // Collections containing resources.  NOTE: this returns collections not resources! Not intended for use in user
    // searches.  This is used when the $collection_search_includes_resource_metadata option is enabled and searches
    // collections based on the contents of the collections.
    if(substr($search, 0, 19) == "!contentscollection")
        {
        // Extract User/Public/Theme flags from the beginning of the search parameter.
        $flags = substr($search, 19, ((strpos($search, " ") !== false) ? strpos($search, " ") : strlen($search)) - 19);
        ...
        // Include the user's own collections.
    ...
    // Duplicate resources (based on file_checksum), find duplicates of a given resource.
    ...
    if(substr($search, 0, 14) == "!relatedpushed")
        ...
        $order_by = str_replace("r.", "", $order_by); # UNION below does not like table aliases in the ORDER BY.
    ...
    if(substr($search, 0, 14) == "!contributions")
        ...
        // Do not filter if user is searching for their own resources and $open_access_for_contributor=true;
    ...
    // Search for a list of resources. !listall = archive state is not applied as a filter to the list of resources.
    ...
    // Within this hook implementation, set the value of the global $sql variable.  Since there will only be one
    // special search executed at a time, only one of the hook implementations will set the value. So, you know that
    // the value set will always be the correct one (unless two plugins use the same !<type> value).
...
*
* IMPORTANT: use resolve_given_nodes() if you need to detect nodes based onsearch string format (ie. @@253@@255 and/ or !@@260)
*
* @param string $string
*
* @return array
...
function search_get_previews($search, $restypes = "", $order_by = "relevance", $archive = 0, $fetchrows = -1, $sort = "DESC", $access_override = false, $starsearch = 0, $ignore_filters = false, $return_disk_usage = false, $recent_search_daylimit = "", $go = false, $stats_logging = true, $return_refs_only = false, $editable_only = false, $returnsql = false, $getsizes = array(), $previewextension = "jpg")
    {
    // Search capability. Note the subset of the available parameters. We definitely do not want to allow override of permissions or filters.
