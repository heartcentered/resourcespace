...
if (!RUNNING_ASYNC)
    ...
    if (empty($_SERVER['argv'][1]) || $scramble_key !== $_SERVER['argv'][1]) 
        {
        exit("Incorrect scramble_key");
        }
    
    if (empty($_SERVER['argv'][2])) 
        {
        exit("Ref param missing");
        }
    $ref = $_SERVER['argv'][2];
    
    if (empty($_SERVER['argv'][3])) 
        {
        exit("File param missing");
        }
    $file = $_SERVER['argv'][3];
    
    if (empty($_SERVER['argv'][4]))
        {
        exit("Target param missing");
        }
    $target = $_SERVER['argv'][4];
    
    if (!isset($_SERVER['argv'][5])) 
        {
        exit("Previewonly param missing");
        }
    $previewonly = $_SERVER['argv'][5];
    
    if (!isset($_SERVER['argv'][6]))
        {
        exit("Snapshottime param missing");
        }
    $snapshottime = $_SERVER['argv'][6];

    if (!isset($_SERVER['argv'][7])) 
        {
        exit("Alternative param missing"); }
...
if (is_array($tmp) and $tmp) 
    {
    list($width, $height) = $tmp;
    }

// Do not proceed, replacetranscode hook intends to avoid everything below.
...
if($video_preview_hls_support != 0)
    ...
    // Windows systems have a hard time with the long paths used for video generation. This workaround creates a batch file containing the command, then executes that.
    if ($config_windows)
        {
        $tmp_ffmpeg_file = get_temp_dir() . "/ffmpeg_" . $ref . "_" . uniqid() . ".bat";
        file_put_contents($tmp_ffmpeg_file, $shell_exec_cmd);
...
if (isset($ffmpeg_alternatives))
    ...
    $ffmpeg_alternatives_count = count($ffmpeg_alternatives);
    for($n = 0; $n < $ffmpeg_alternatives_count; $n++)
        {
        $generate = true;
        ...
        // OK to generate this alternative?
        if ($generate) 
            {
        ...
        if(isset($qtfaststart_path))
            ...
            else // Remove the alternative file entries with this name as ffmpeg has failed to create file.
                {
                // SQL Connection may have hit a timeout.
                sql_connect();
                $existing = sql_query("SELECT ref FROM resource_alt_files WHERE resource='$ref' and name='" . escape_check($ffmpeg_alternatives[$n]["name"]) . "'");
