...
function migrate_resource_type_field_check(&$resource_type_field)
    {
    if(!isset($resource_type_field['options']) || is_null($resource_type_field['options']) ||
        $resource_type_field['options']=='' || ($resource_type_field['type'] == 7 && preg_match('/^' . MIGRATION_FIELD_OPTIONS_DEPRECATED_PREFIX_CATEGORY_TREE . '/',$resource_type_field['options'])) ||
    ...
    // Delete all nodes for this resource type field.  This is to prevent systems that migrated to have old values
    // that have been removed from a default field.  example: Country field
    delete_nodes_for_resource_type_field($resource_type_field['ref']);

    // Category tree.
    if($resource_type_field['type'] == 7)
        {
...
// Populate resource_node with all resources that have resource_data matching .  Also get hit count from
// resource_keyword if the normalised keyword matches.
...
function migrate_search_filter($filtertext)
    ...
    foreach($filter_rules as $filter_rule)
        ...
        // This will contain the SQL value sets to be inserted for this rule.
        $nodeinsert = array();
...
function mix_date($fromdate, $maxoffset = 30)
    ...
    // How much should dates be moved?
    $dateshift = 86400 * $maxoffset; # 60*60*24
...
* @param boolean $recurse     Optionally prevent recursion (maybe called by another mix function).
* @return string
*/
    ...
    if($bytelength == $mbytelength && (string)(int)$string == $string)
        ...
        for($i = 0; $i < $mbytelength; $i++)
            ...
            if($i > 3 && strpos($noreplace, $oldchar) === false)
                ...
                elseif($randaction == 1) // Add a character.
                    {
                    --$i;
                    }
                }
...
function mix_url($string)
    ...
    if(strpos($string, "pages") === 0 || strpos($string, "/pages") === 0 || strpos($string, $baseurl) === 0)
        {
        // URL is a relative path within the system, do not scramble.
        return $string;
        }
