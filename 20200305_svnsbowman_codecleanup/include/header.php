...
// Do not display header and footer when dynamically loading CentralSpace contents.
$ajax = getval("ajax", "");
...
if($ajax == "" && !hook("replace_header"))
    ...
    <?php if (($top_nav_upload && checkperm("c")) || ($top_nav_upload_user && checkperm("d")))
        { ?>
        <li class="HeaderLink UploadButton"><a href="<?php echo $baseurl; if ($upload_then_edit) { ?>/pages/upload_plupload.php<?php } else { ?>/pages/edit.php?ref=-<?php echo @$userref?>&amp;uploader=<?php echo $top_nav_upload_type; } ?>" onClick="return CentralSpaceLoad(this,true);"><?php echo UPLOAD_ICON ?><?php echo $lang["upload"]?></a></li><?php
        } ?>
...
// Update header links to add a class that indicates current location. We parse URL for systems that are one level
// deep under web root.
