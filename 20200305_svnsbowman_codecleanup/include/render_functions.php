<?php
/**
* Functions Used to Render HTML and Javascript
*
* @package ResourceSpace
*/

/**
* Renders the HTML for the provided $field for inclusion in a search form, for example the advanced search page.
...
function render_search_field($field, $value = "", $autoupdate, $class = "stdwidth", $forsearchbar = false, $limit_keywords = array(), $searched_nodes = array(), $reset = "")
    ...
    if(!$forsearchbar)
        ...
        <?php
        // Hook to modify field type in special case. Returning zero (to get a standard text box) does not work, so return 1 for type 0, 2 for type 1, etc.
    ...
    switch ($field["type"])
        ...
        // Number view, manipulate the form value (do not send these but send a compiled numrange value instead).
        elseif($field['field_constraint'] == 1) # Parse value for to/from simple search.
            {
        ...
        if(!hook("customchkboxes", "", array($field, $value, $autoupdate, $class, $forsearchbar, $limit_keywords)))
            ...
            // Show a check list or dropdown for dropdowns and check lists? By default show a checkbox list for both
            // (for multiple selections this enabled OR functionality).
            ...
            foreach($field['nodes'] as $node_index => $node)
                {
                // Special case for vertically ordered checkboxes.  Order by needs to be reset as per the new order so
                // that we can reshuffle them using the order by as a reference.
            ...
            if($field["display_as_dropdown"] || $forsearchbar)
            ...
            else // Show as a checkbox list (default).
                {
                ...
                else // Horizontal Ordering (Standard).
                    { ?>
                    <table cellpadding=2 cellspacing=0>
                        <tr>
                    <?php
        ...
        case FIELD_TYPE_DATE_RANGE:
            ...
            if (!$forsearchbar && $daterange_search)
                ...
                <?php if ($forsearchbar && $searchbyday)
                    {
                    ?><br /><?php
                    } ?>
        ...
        */
        ...
        else // For advanced search and elsewhere, include the category tree.
            {
            include __DIR__ . "/../pages/edit_fields/7.php";
            }
        break;
...
if (!function_exists("render_sort_order"))
    ...
    function render_sort_order(array $order_fields, $default_sort_order)
        ...
        // Use query strings here as this is used to render elements and sometimes it can depend on other parameters.
        ...
        foreach($order_fields as $name => $label)
            {
            // Date shows as 'field'.$date_field rather than 'date' for collection searches, so fix it.
...
if(!function_exists("render_actions"))
    ...
    function render_actions(array $collection_data, $top_actions = true, $two_line = true, $id = '',$resource_data = array(), $optionsonly = false, $forpage = "")
        ...
        $options = '';
        $lastcategory = 0;
        $actions_array_count = count($actions_array);
        for($a = 0; $a < $actions_array_count; $a++)
            {
        ...
        function action_onchange_<?php echo $action_selection_id; ?>(v)
            ...
            if(!$top_actions || !empty($collection_data))
                ...
                if(substr($search, 0, 11) == '!collection')
                    {
                    // The collection search must always be the first part of the search string.
                    $search_trimmed = substr($search,11);
                ...
                // Add extra collection actions javascript case through plugins.  Note: If you are just going to a
                // different page, it should be easily picked by the default case.
...
* @param string $label       Label of question.
* @param string $input       Name of input field.
* @param array  $options     Array of options (value and text pairs) (eg.
*                             array('pixelwidthmin'=>'From','pixelwidthmin'=>'To')
*/
...
// NOTE: Functions is_field_displayed, display_multilingual_text_field and display_field below moved from edit.php.
function is_field_displayed($field)
    ...
    return !(($resource["archive"] == 0 && $field["resource_type"] == 999)
        ...
        && !($ref < 0 && checkperm("P" . $field["ref"]))) || checkperm("F" . $field["ref"])
        // Upload only field.
...
function display_field($n, $field, $newtab = false,$modal = false)
    ...
    $all_selected_nodes,$original_nodes, $FIXED_LIST_FIELD_TYPES, $TEXT_FIELD_TYPES, $upload_review_mode, $check_edit_checksums, $upload_review_lock_metadata, $locked_fields, $lastedited, $copyfrom, $fields;
    ...
    // Return this field value back to the original value, instead of using the value from the copied
    // resource/metadata template, this is triggered if field has the 'omit_when_copying' flag set.
    ...
    if($multiple && ((getval("copyfrom", "") == "" && getval('metadatatemplate', '') == "") || str_replace(array(" ", ","), "", $value) == ""))
        {
        $value = "";
        }
    ...
    // For batch editing, CKEditor renders as a text box, as it does not work at all well when appending / prepending
    // (it expects to work with HTML only).
    ...
    // Define some JavaScript for help actions (applies to all fields).  Help actions for CKEditor fields are set in
    // pages/edit_fields/8.php.
    if(trim($field["help_text"] == ""))
        ...
        else // For all other field types setup javascript to toggle helptext depending on loss or gain of focus.
            {
            $help_js = "onBlur=\"HideHelp(" . $field["ref"] . ");return false;\" onFocus=\"ShowHelp(" . $field["ref"] . ");return false;\"";
            }
        }

    // Hook to modify field type in special case. Returning zero (to get a standard text box) does not work, so return
    // 1 for type 0, 2 for type 1, etc.
    $modified_field_type = "";
    ...
    // Show inline help for this field.  For certain field types that have no obvious focus, the help always appears.
    if(trim($field["help_text"] != ""))
        { ?>
...
function render_date_range_field($name, $value, $forsearch = true, $autoupdate = false, $field = array(), $reset = "")
    ...
    // Get the start and end date from the string.
    if($forsearch)
        {
...
*   are being created and make sure they all use this function [or any future related functions - like
*   generateBreadcrumb()].
*
...
function render_resource_image($imagedata, $img_url, $display="thumbs")
    ...
    else // Use php function getimagesize().
        {
    ...
    switch($display)
        ...
        elseif ($ratio < 1) // Portrait image dimensions.
            {
            $height = $defaultheight;
            $width = round($defaultwidth * $ratio);
            $margin = floor(($defaultheight - $height ) / 2) . "px";
            }
        else // Square image or no image dimensions.
            {
            $height = $defaultheight;
            $width = $defaultwidth;
            $margin = "auto";
...
function render_share_options($collectionshare = true, $ref, $emailing = false)
    ...
    <?php endif; // End hook replaceemailaccessselector.
    ...
    // Allow the selection of a user group to inherit permissions from for this share (the default is to use the
...
* @param boolean $return_params_only Exception to the rule! Rather than render, return the upload here parameters.
*
* @return void
*/
function render_upload_here_button(array $search_params, $return_params_only = false)
    ...
    if(isset($search_params['search']) && (mb_substr($search_params['search'], 0, 11) != '!collection'
    ...
    // Option to return out just the upload parameters.
...
* @param boolean $return_string     Set to true to return the html as a single line string, False will cause the
*                                    function to echo the HTML.
*
...
function render_help_link($page = '', $return_string = false)
    ...
    if($contextual_help_links === false)
        {
        return;
        }
