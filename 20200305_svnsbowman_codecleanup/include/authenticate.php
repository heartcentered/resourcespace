...
if (!function_exists("ip_matches"))
    ...
    function ip_matches($ip, $ip_restrict)
        ...
        for ($n = 0;$n < $i_count; $n++)
            ...
            else // No wildcard, straight match.
                {
                if ($ip == $ip_restrict)
                    {
                    return true;
...
if (array_key_exists("user", $_COOKIE) || array_key_exists("user", $_GET) || isset($anonymous_login) || hook('provideusercredentials'))
    ...
    $userdata = validate_user($user_select_sql, true);
    ...
    if(count($userdata) > 0)
        ...
        if($password_expiry > 0 && !checkperm("p") && $allow_password_change && $pagename != "user_change_password" && $pagename != "index" && $pagename != "collections" && strlen(trim($userdata[0]["password_last_change"])) > 0 && getval("modal", "") == "")
            ...
            if ($last_password_change > ($password_expiry * 86400)) # 60*60*24
                { ?>
                <script>
        ...
        if (!isset($system_login) && strlen(trim($userdata[0]["last_active"])) > 0)
            {
            // Last active more than $session_length minutes ago?
            if ($userdata[0]["idle_seconds"] > ($session_length * 60))
                ...
                // If auto logout enabled, but this is not the anonymous user, log them out.
                if ($session_autologout && $username != $al)
                    {
                    // Reached the end of valid session time, auto log out the user. Remove session.
                ...
                else // Session end reached, but the user may still remain logged in, but this is a new session for the purposes of statistics.
                    {
    ...
    // Set a cookie that we will check for again on the login page after the redirection. If this cookie is missing,
    // it is assumed that cookies are switched off or blocked and a warning message is displayed.
...
if (!$valid && !isset($system_login))
    ...
    // Disable forwarding of the AJAX parameter if this was an AJAX load; otherwise, the redirected page will be missing the header/footer.
    $path = str_replace("ajax=", "ajax_disabled=", $path);
...
Note: it is considered safe to show the collection bar because even if we enable login terms later on, when the user
      might have resources in it, they would not be able to do anything with them unless they accept terms.
*/
...
// Do not update this table if the system is doing its own operations.
...
// Checks user has opted to see the full site view, rather than the responsive version on a device.
...
if($_SERVER["REQUEST_METHOD"] === "POST" && !isValidCSRFToken($csrf_token, $usersession) && !(isset($anonymous_login) && $username == $anonymous_login))
    {
