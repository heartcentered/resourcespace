...
if(getval("create","") != "" && $ref == 0 && $userref > 0) // Saves manual link creation having to work out user template ref.
    {
    $ref = 0 - $userref;
    }
...
if($tabs_on_edit || $upload_review_mode)
    {
...
$metadatatemplate = !$resetform ? (getvalescaped('metadatatemplate', ($metadata_template_default_option == 0 ? 0 : $metadata_template_default_option),
    true)) : $metadata_template_default_option;
...
<form method="post"
   ...
   elseif ($ref>0)
        ...
        if (!hook("replaceeditheader"))
            ...
            <?php
            # Draw nav
         if (!$multiple  && $ref>0  && !hook("dontshoweditnav"))
            {
            EditNav();
            }
...
if ($edit_upload_options_at_top || $upload_review_mode)
    {
...
if (!$multiple && !$modal && $ref>0 && !hook("dontshoweditnav"))
    {
    EditNav();
    }
