...
if(getval("submit","") != "")
    ...
    $csvdelimiter = (getvalescaped('csvdelimiter', '') != '');
    $extension = (getvalescaped('extension', '') != '');
    ...
    if (!hook('csvreplaceheader') && !$extension)
        {
        ...
        {
        header("Content-type: application/octet-stream");
        header("Content-disposition: attachment; filename=search_results_metadata.txt");
        }

    echo generateResourcesMetadataCSV($search_results, $personaldata, $allavailable, $csvdelimiter, $linestrip);
    exit();
    }
else
    {
    ...
    <div class="BasicsBox">
        ...
        <form method="post" action="<?php echo $baseurl_short?>pages/csv_export_results_metadata.php" >
            ...
            <div class="Question" id="question_delimiter">
                <label for="allavailable"><?php echo htmlspecialchars($lang['csvExportResultsDelimiter']) ?></label>
                <input name="csvdelimiter" id="csvdelimiter" type="checkbox" value="true" style="margin-top:7px;">
                <div class="clearerleft"> </div>
            </div>

            <div class="Question" id="question_extension">
                <label for="allavailable"><?php echo htmlspecialchars($lang['csvExportResultsExtension']) ?></label>
                <input name="extension" id="extension" type="checkbox" value="true" style="margin-top:7px;">
                <div class="clearerleft"> </div>
            </div>
