<?php
// Installation Check, A Simple Script to Check the ResourceSpace Hosting Environment Supports our Needs
...
function ResolveKB($value)
    ...
    if(substr($value, -1, 1) == "G")
        {
        return substr($value, 0, strlen($value) - 1) * 1048576; # 1024*1024
        }
    return $value;
    }
...
// Check the ResourceSpace version. Should not be translated as this information is sent to the bug tracker.
$p_version = $productversion == 'SVN' ? 'Subversion ' . $build : $productversion; 
...
// Check PHP.INI values for memory_limit, post_max_size, and upload_max_filesize.
$memory_limit = ini_get("memory_limit");
if(ResolveKB($memory_limit) < (204800)) # 200*1024
    {
...
if(ResolveKB($post_max_size) < (102400)) # 100*1024
    {
    $result = $lang["status-warning"] . ": " . str_replace("?", "100M", $lang["shouldbeormore"]);} else {$result=$lang["status-ok"];
    }
...
if(ResolveKB($upload_max_filesize) < (102400)) # 100*1024
    {
    $result = $lang["status-warning"] . ": " . str_replace("?", "100M", $lang["shouldbeormore"]);} else {$result=$lang["status-ok"];
        }
...
// Check if the PHP EXIF extension is installed.
...
// List all of the installed PHP extensions.
?>
<tr>
...
// Check for write access to filestore
...
// Check for write access to homeanim if transform plugin is installed.
...
?><tr><td colspan="2"><?php echo $lang["writeaccesstohomeanim"] ?></td><td><b><?php echo $result?></b></td></tr><?php
    } 

// Check for filestore folder browseability.
...
if($aws_s3)
    ...
    // Check if the AWS S3 bucket is accessible.
    $result = aws_s3_bucket_check($aws_bucket);

    // Determine the AWS S3 bucket location and ACL information.
    $result2 = aws_s3_bucket_location($aws_bucket);
    $result3 = aws_s3_bucket_owner($aws_bucket);

    ?><tr><td><?php echo $lang["aws_s3"]; ?></td><td><?php echo $lang['aws_s3_bucket'] . $aws_bucket ?><br/><?php echo $lang['aws_s3_region'] . $result2['location']?><br/><?php echo $lang['aws_s3_owner'] . $result3['name'] ?><br/><?php echo $lang['aws_s3_id'] . $result3['id'] ?></td><td><b><?php echo $result?></b></td></tr><?php
    }

// Check if ImageMagick/GraphicsMagick is installed.
...
// Check the archiver tool.
if(!$use_zip_extension)
    {
    // Only check if it is going to be used.
    if ($collection_download || isset($zipcommand)) 
        {
...
<tr>
    ...
    <td><?php $last_cron = sql_value("SELECT datediff(now(),value) value FROM sysvars WHERE name='last_cron'", $lang["status-never"]);
    echo $last_cron ?></td>
...
function display_extension_status($extension)
    ...
    ?><tr><td colspan="2">PHP-<?php echo $extension . " " . $lang['php_extension']?></td>
    <td><b><?php echo $result?></b></td></tr><?php
    }    
...
function get_utility_version($utilityname)
    ...
    // There was a path but it was incorrect, the utility could not be found.   
    ...
    else // There was a working path and the output was the expected, the version is returned.    
        {
        $s = explode("\n", $version);
