...
// Function to update geolocation for a resource.
function api_update_geolocation($ref, $geo_lat, $geo_long)
    {
    // Input data validation.
    if(is_numeric($ref) && is_numeric($geo_lat) && is_numeric($geo_long))
        {
        $map_zoom = map_zoom($geo_lat, $geo_long);
        sql_query("UPDATE resource SET geo_lat='" . escape_check($geo_lat) . "', geo_long='" . escape_check($geo_long) . "', mapzoom='" . escape_check($map_zoom) . "' WHERE ref='" . escape_check($ref) . "'");
...
// Function to enable file upload via mobile apps.
function api_http_upload()
    {
    return http_upload();
    }
...
function http_upload()
    {
    global $lang, $file_upload_abs_path;

    $target_dir = $file_upload_abs_path;
...
// Function to get the current user, but return limited information for security purposes.
function api_get_user()
    {
    global $userref;
    $results = get_user($userref);

    // Only pass specific usergroup fields to the API for security reasons.
    $user_info["ref"] = $results["ref"]; # User ref ID.
    $user_info["username"] = $results["username"]; # User login name.
    $user_info["fullname"] = $results["fullname"]; # User full name.
    $user_info["usergroup"] = $results["usergroup"]; # User's group ref ID.
    $user_info["groupname"] = $results["groupname"]; # User's group name.
    $user_info["comments"] = $results["comments"]; # Comments about the user's group.

    return $user_info;
    }


// Function to get the current user's group, but return limited information for security purposes.
function api_get_user_group()
    {
    global $userref;
    $results = get_user($userref);
    $group_results = get_usergroup($results["usergroup"]); # User's group ref ID.
    
    // Only pass specific usergroup fields to the API for security reasons.
    $group_info["ref"] = $group_results["ref"];
    $group_info["name"] = $group_results["name"];
    return $group_info;
    }
