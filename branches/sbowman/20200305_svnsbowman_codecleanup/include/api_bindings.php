 ...
 *
 * For documentation please see: https://www.resourcespace.com/knowledge-base/api/
 *
 */
...
// Provides simple way to update field by passing in simple string values for text fields, comma separated values for
// fixed list (node) fields, using double quotes to enclose strings and backslash as escape character.
    ...
    if(in_array($fieldinfo['type'], $FIXED_LIST_FIELD_TYPES))
        ...
        if($nodevalues)
            ...
            foreach($fieldnodes as $fieldnode)
                ...
                elseif(in_array($fieldnode["ref"], $current_field_nodes) && !in_array($fieldnode["name"], $sent_nodes))
                    {
            ...
            # If this is a dynamic keyword field, then need to add any new entries to the field nodes.
                ...
                foreach($fieldnodes as $fieldnode)
                    ...
                    if(count($fieldoptiontranslations) < 2)
                        ...
                        for ($n = 1; $n < $fieldoptiontranslations_count; $n++)
                            ...
                            else // Support both 2 character and 5 character language codes (for example en, en-US).
                                {
        ...
        if(count($nodes_to_add) > 0 || count($nodes_to_remove) > 0)
            ...
            // Update log, first use the node array to getnames with node ID as key.
...
function api_set_node($ref, $resource_type_field, $name, $parent = '', $order_by = 0, $returnexisting = false)
    ...
    if(!in_array($fieldinfo['type'], $FIXED_LIST_FIELD_TYPES) ||
       !(checkperm('a') || checkperm('k') || ($fieldinfo['type'] == FIELD_TYPE_DYNAMIC_KEYWORDS_LIST && !checkperm('bdk' . $resource_type_field))))
        {
        // API user does not have permission to add new nodes.
        return false;
        }
...
function api_get_node_id($value, $resource_type_field)
    ...
    if(!metadata_field_view_access($resource_type_field))
        {
        return false; # Need at least view access to the field.
        }
...
function api_replace_resource_file($ref, $file_location, $no_exif = false, $autorotate = false, $keep_original = true)
    ...
    if (count($duplicates) > 0)
        ...
        else // Set global option so that this is not dependent on config.
            {
