...
DEFINE("MESSAGE_DEFAULT_TTL_SECONDS", 604800); # 7 days (60*60*24*7)

// Gets messages for a given user (return true if there are messages, if not false), note that messages are passed by reference.
...
function message_add($users, $text, $url = "", $owner = null,$notification_type = MESSAGE_ENUM_NOTIFICATION_TYPE_SCREEN, $ttl_seconds = MESSAGE_DEFAULT_TTL_SECONDS, $related_activity = 0, $related_ref = 0)
    ...
    foreach($users as $user)
        ...
        // Send an email if the user has notifications and emails setting and the message has not already been sent via email.
...
function message_send_unread_emails()
    ...
    if($inactive_message_auto_digest_period > 0 && is_numeric($inactive_message_auto_digest_period))
        ...
        foreach($allusers as $user)
            {
            if(!in_array($user["ref"], $digestusers) && strtotime($user["last_active"]) < date(time() - $inactive_message_auto_digest_period * 86400)) # 24*60*60
                {
    ...
    foreach($digestusers as $digestuser)
        ...
        // This may be set differently as a group configuration overerride or disabled by user.
        if($inactive_message_auto_digest_period == 0 || (!$user_pref_inactive_digest && !$user_pref_daily_digest))
            {
