...
function get_user_actions($countonly = false, $type = "", $order_by = "date", $sort = "DESC")
    ...
    if($hookactionsql != false)
        {
        $actionsql = (($actionsql != "") ? $actionsql . " UNION " : "") . $hookactionsql;
        }
    ...
    if ($countonly)
        ...
        $actionsql = "SELECT date, allactions.ref,user.fullname as
            user," . ($messages_actions_usergroup?"usergroup.name as
            usergroup,":"") . " description,
            type FROM (" . $actionsql . ")  allactions LEFT JOIN user ON
            allactions.user=user.ref" . ($messages_actions_usergroup?" LEFT JOIN usergroup ON
            user.usergroup=usergroup.ref":"") . " ORDER BY " . $order_by . " " . $sort;
        }
