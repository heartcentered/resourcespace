...
function check_api_key($username, $querystring, $sign)
    ...
    if ($s === false || $s + 6 + strlen($sign) !== strlen($querystring))
        {
        return false;
        }
...
function execute_api_call($query, $pretty = false)
    ...
    foreach ($fct->getParameters() as $fparam)
        ...
        elseif ($fparam->isOptional()) // Set default value if nothing passed, such as from the API test tool.
            {
...
function iiif_get_canvases($identifier, $iiif_results, $sequencekeys = false)
    ...
    foreach ($iiif_results as $iiif_result)
        ...
        // If the largest image dimensions are less than 1200 pixels on either edge, then the canvas dimensions should be double those of the image - From http://iiif.io/api/presentation/2.1/#canvas
...
* @param array $size          ResourceSpace size information. Required information: identifier and whether it requires
*                             to return height and width back (e.g annotations do not require it).
...
function iiif_get_image($identifier, $resourceid, $position, array $size_info)
    ...
    // Quick validation of the size_info parameter.
