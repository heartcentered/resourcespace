...
* @return string
*/
function get_resource_path($ref, $getfilepath, $size = '', $generate = true, $extension = 'jpg', $scramble = true, $page = 1, $watermarked = false, $file_modified = '', $alternative = -1, $includemodified = true)
    {
    ...
    if(!$getfilepath && $hide_real_filepath)
        ...
        if(!isset($get_resource_path_extra_download_query_string_params)
    ...
    if($size == "")
        ...
        if(!isset($get_resource_path_fpcache[$ref]))
            {
            $get_resource_path_fpcache[$ref] = sql_value("SELECT file_path value FROM resource where ref='" . escape_check($ref) . "'","");}
    ...
    if($originals_separate_storage)
        ...
        elseif($size == "") // Original file (core file or alternative).
            {

            $path_suffix = "/original/";
            }
        else // Preview or thumb.
            {
            $path_suffix = "/resized/";
            }
        }
...
// Returns basic resource data (from the resource table alone) for resource $ref.  For 'dynamic' field data, see
...
function get_resource_data($ref, $cache = true)
    ...
    if(count($resource) == 0)
        ...
        else // For upload templates (negative reference numbers), generate a new resource if upload permission.
            {
...
// Returns field data and field properties (resource_type_field and resource_data tables) for this resource, for
// display in an edit / view form.  Standard field titles are translated using $lang.  Custom field titles are i18n translated.
if(!function_exists('get_resource_field_data'))
    ...
    function get_resource_field_data($ref, $multi = false, $use_permissions = true, $originalref = NULL, $external_access = false, $ord_by = false)
        ...
        // Resource types can be configured to not have global fields in which case we only present the user fields valid for this resource type.
        ...
        if(!$inherit_global_fields && !$multi)
            ...
            for($n = 0; $n < $fields_count; $n++)
                {
                if($fields[$n]['ref'] == $view_title_field # Check field against $title_field for default title reference.
...
// Returns field data and field properties (resource_type_field and resource_data tables) for all the resource
// references in the array $refs.  This will use a single SQL query and is therefore a much more efficient way of
// gathering resource data for a list of resources (e.g. search result display for a page of resources).
...
// Returns a list of resource types. The standard resource types are translated using $lang. Custom resource types
// are i18n translated. Support getting info for a comma-delimited list of restypes (as in a search).
...
// Return the top $count keywords (by hitcount) used by $resource. This section is for the 'Find Similar' search.
// These are now derived from a join of node and resource_node for fixed keyword lists and resource_data for free
// text fields.  Currently, the date fields are not used for this feature.
function get_resource_top_keywords($resource, $count)
    ...
    foreach($combined as $keyword)
        ...
        else // In this case the keyword is coming from nodes.
            {
            // Apply permissions and strip out any results the user does not have access to.
        ...
        if(isset($r) && trim($r) != '')
            ...
            // Splitting keywords can result in break words being included in these results. These should be removed
            // here otherwise they will show as keywords themselves which is incorrect.
            $s = split_keywords($r);
...
if(!function_exists("split_keywords"))
    ...
    function split_keywords($search, $index = false, $partial_index = false, $is_date = false, $is_html = false, $keepquotes = false)
        ...
        // Remove any real or unescaped LF or CR.
        ...
        if($is_html || (substr($search,0,1) == "<" && substr($search, -1, 1) == ">"))
            {
            // String cannot be in encoded format at this point or string will not be indexed correctly.
            $search = html_entity_decode($search);
            // Clean up HTML for indexing, allow indexing of anchor text.
            if($index)
                {
        ...
        // Special 'constructed' query type, split using comma so we support keywords with spaces.
        if((substr($ns, 0, 1) == ",") || ($index == false && strpos($ns, ":") !== false))
            {
            if(!$index && $keepquotes)
                {
            ...
            // If we are indexing, append any values that contain spaces. Important! Solves the searching for keywords
            // with spaces issue. Consider: for any keyword that has spaces, append to the array each individual word to
            // so for example: South Asia,USA becomes South Asia,USA,South,Asia so a plain search for 'south asia' will
            // match those with the keyword 'south asia' because the resource will also be linked to the words 'south' and 'asia'.
            if($index)
                {
                $return2 = $return;
            ...
            else // If we are not breaking quotes, we may end up a with commas in the array of keywords which need to be removed.
                {
                return trim_array($return, $config_trimchars . ($keepquotes ? "," : ""));
                }
            }
        else // Split using spaces and similar chars (according to configured whitespace characters).
            {
...
if(!function_exists("cleanse_string"))
    ...
    function cleanse_string($string, $preserve_separators, $preserve_hyphen = false, $is_html = false)
        ...
        // Replace some HTML entities with empty space, most of them should already be in $config_separators but
        // others, like &shy; do not have an actual character that we can copy and paste to $config_separators.
        ...
        if(substr($string, 0, 1) == "!" && strpos(substr($string, 1), "!") === false)
            {
            // If we have the exclamation mark configured as a config separator but we are doing a special search, we do not want to remove it.
        ...
        else // Also strip out the separators used when specifying multiple field/keyword pairs (comma and colon).
            {
            $s = $separators;
            $s[] = ",";
            $s[] = ":";
...
// For each keywords in the supplied keywords list, add all possible infixes and return the combined array. This
// therefore Returns all keywords that need indexing for the given string.  Only for fields with 'partial_index' enabled.
function add_partial_index($keywords)
    ...
    for($n = 0; $n < $keywords_count; $n++)
        ...
        // Do not do this for keywords containing spaces as these have already been broken to individual words using the code above.
        if(strpos($keyword, " ") === false)
            {
...
// For the specified $resource, increment the hitcount for each matching keyword in $search.  This is done into a
// temporary column first (new_hit_count) so existing results are not affected.  copy_hitcount_to_live() is then
// executed at a set interval to make this data live.
...
function copy_hitcount_to_live()
    ...
    // Also update the resource table. greatest() is used so the value is taken from the hit_count column in the
    // event that new_hit_count is zero to support installations that did not previously have a new_hit_count column (i.e. upgrade compatability).
...
// Returns a table of available image sizes for resource $ref. The standard image sizes are translated using $lang.
// Custom image sizes are i18n translated. The original image file assumes the name of the 'nearest size (up)' in the table.
        ...
        for($n = 0; $n < $sizes_count; $n++)
            ...
            if(($file_exists || (!$onlyifexists)) && !checkperm("T" . $resource_type . "_" . $sizes[$n]["id"]))
                ...
                if(($sizes[$n]["internal"] == 0) || ($internal))
                    ...
                    else // Take the restriction from the settings for this download size.
                        {
...
/**
* Get the resource data value for a field and a specific resource or get the specified field for all resources in
* the system.
*
* @param integer        $resource Resource ID. Use NULL to retrieve all resource records for the specified field.
...
if(!function_exists("get_users"))
    ...
    function get_users($group = 0, $find = "", $order_by = "u.username", $usepermissions = false, $fetchrows = -1, $approvalstate = "", $returnsql = false, $selectcolumns = "")
        ...
        for($n = 0; $n < $r_count; $n++)
            {
            // The padded rows cannot be and do no need to be translated.
...
if(!function_exists('save_user'))
    ...
    function save_user($ref)
        ...
        if('' != getval('deleteme', ''))
            ...
            $email = trim(getval('email', '')); # To be escaped on usage in database.
            ...
            // Make sure IP restrict filter is a proper IP, otherwise make it blank.  We do this check only when wildcards are not used.
        ...
        // Add user group dash tiles as soon as we have changed the user group.
        if($home_dash)
            {
...
if(!function_exists("auto_create_user_account"))
    ...
    function auto_create_user_account($hash = "")
        {
        global $applicationname, $user_email, $baseurl, $email_notify, $lang, $user_account_auto_creation_usergroup, $registration_group_select, $auto_approve_accounts, $auto_approve_domains, $customContents, $language, $home_dash;
        ...
        elseif(count($auto_approve_domains) > 0)
            ...
            foreach($auto_approve_domains as $domain => $set_usergroup)
                {
                // If a group is not specified the variables, do not get set correctly so we need to correct this.
        ...
        if($approve)
           ...
           global $usergroup, $user_pref_user_management_notifications, $email_user_notifications;
           ...
           foreach($approval_notify_users as $approval_notify_user)
            ...
            if(count($message_users) > 0)
                {
                message_add($message_users, $notificationmessage, $templatevars['linktouser'], $new, MESSAGE_ENUM_NOTIFICATION_TYPE_SCREEN, 2592000, USER_REQUEST, $new ); # 60*60*24*30
                }
            }
...
function email_user_request()
    ...
    if(0 < count($message_users))
        {
        // Send a message with long timeout (30 days).
        message_add($message_users, $notificationmessage, '', 0, MESSAGE_ENUM_NOTIFICATION_TYPE_SCREEN, 2592000); # 60*60*24*30
        }
...
function get_all_site_text($findpage = "", $findname = "", $findtext = "")
    ...
    // English should always be included, as it is the fallback language of the system.
    ...
    if('' != $findtext)
        ...
        foreach($search_languages as $search_language)
            ...
            foreach($lang as $key => $text)
                ...
                // Do not support overrides for array values (used for months), complex UI needed and very unlikely to need overrides.
                if(!is_array($text) && ($findname == "" || stripos($key, $findname) !== false)
                    ...
                    // Make sure this is not already set for default/another language.
        ...
        if($findtext != "" || $findpage != "" || $findname != "")
            ...
            if($findtext != "")
                {
                $search = "text like '%" . escape_check($findtext) . "%'";
                }
            if($findpage != "")
                {
                $search = "page like '%" . escape_check($findpage) . "%'";
                }
            if($findname != "")
                {
                $search = "name like '%" . escape_check($findname) . "%'";
                }
            ...
            foreach($site_text as $text)
                ...
                // Make sure we do not include the default if we have overwritten.
...
function get_site_text($page, $name, $getlanguage, $group)
    {
    // Registering plugin text uses $language and $lang, others needed as they are referenced in lang files.
    global $defaultlanguage, $lang, $language, $applicationname, $storagedir, $homeanim_folder;
...
function formatfilesize($bytes)
    ...
    global $byte_prefix_mode_decimal, $lang;
...
function send_mail($email, $subject, $message, $from = "", $reply_to = "", $html_template = "", $templatevars = null, $from_name = "", $cc = "", $bcc = "")
    {
    // NOTE: $from is the name of the user sending the email, while $from_name is the name that should be put in the
    // header, which can be the system name.  It is necessary to specify two since in all cases the email should be
    // able to contain the user's name.  Old mail function remains the same to avoid possible issues with phpmailer
    // send_mail_phpmailer allows for the use of text and html (multipart) emails, and the use of email templates in
    // Manage Content.
    ...
    IMPORTANT: FILTER_VALIDATE_EMAIL is not fully RFC 2822 compliant, an email like "Another User <anotheruser@example.com>" will be invalid.
    */
    ...
    // Send an email, but correctly encode the message/subject in quoted-printable UTF-8.
    ...
    // Include footer.
    global $email_footer, $disable_quoted_printable_enc;

    // Work out correct EOL to use for emails (should use the system EOL).
    if(defined("PHP_EOL"))
        {
        $eol = PHP_EOL;
        }
    ...
    $reply_tos_count = count($reply_tos);
    for($n = 0; $n < $reply_tos_count; $n++)
        {
        if($n != 0)
            {
            $headers .= ",";
    ...
    if($bcc != "")
        ...
        // Add bcc.
        $bccs = explode(",", $bcc);
        $bccs_count = count($bccs);
        $headers .= "Bcc: ";
        for($n = 0; $n < $bccs_count; $n++)
            {
            if($n != 0)
                {
                $headers .= ",";
...
// If ($use_phpmailer==true), this function is used instead.  Mail templates can include lang, server, site_text,
// and POST variables by default, ex ( [lang_mycollections], [server_REMOTE_ADDR], [text_footer] , [message]
// additional values must be made available through $templatevars.  For example, a complex url or image path that
// may be sent in an email should be added to the templatevars array and passed into send_mail.  Available
// templatevars need to be well-documented, and sample templates need to be available.
    ...
    function send_mail_phpmailer($email, $subject, $message = "", $from = "", $reply_to = "", $html_template = "", $templatevars = null, $from_name = "", $cc = "", $bcc = "")
        ...
        if($html_template != "")
            ...
            if(array_key_exists($language . "-" . $html_template,$site_text))
                ...
                // Cannot find the language key? Look for it in other languages.
                reset($languages);
            ...
            if(isset($template) && $template != "")
                ...
                foreach($test[0] as $variable)
                    ...
                    elseif(substr($variable, 0, 15) == "img_storagedir_")
                        {
                    ...
                    elseif('img_headerlogo' == substr($variable, 0, 14))
                        {
                        $img_url = get_header_image(true);
                        $$variable = '<img src="' . $img_url . '"/>';
                        }
                    elseif('img_' == substr($variable, 0, 4))
                        {
                    ...
                    elseif(substr($variable, 0, 7) == "attach_")
                        {
                        $$variable = "";
                        $attachments[] = substr($variable, 7);
                        }

                    // Get site text variables (ex. [text_footer], for example to manage HTML snippets that you want available in all emails.)
                    elseif(substr($variable, 0, 5) == "text_")
                        {
        ...
        // If there are multiple addresses, that is what replyto handles.
        $reply_tos_count = count($reply_tos);
        for($n = 0; $n < $reply_tos_count; $n++)
            {
            if(strstr($reply_tos[$n],"<"))
        ...
        // Modification to handle multiple comma delimited emails, such as for a multiple $email_notify.
        $emails = $email;
        ...
        // Modification for multiple is also necessary here, though a broken bcc seems to be simply removed by phpmailer rather than breaking it.
        if($bcc != "")
            {
            $bccs = $bcc;
            $bccs = explode(',', $bccs);
...
// Log email, data logged is: time, to address, from, user ID or 0 for system emails (cron etc.), and subject.
...
// Quoted printable encoding is rather simple.  Each character in the string $string should be encoded if:
// Code is <0x20 (space), char is = (as it has a special meaning: 0x3d), and is over ASCII range (>=0x80).
function rs_quoted_printable_encode($string, $linelen = 0, $linebreak = "=\r\n", $breaklen = 0, $encodecrlf = false)
    ...
    for($i = 0; $i < $len; $i++)
        ...
        // Normal characters are not encoded.
        $result .= chr($c);
        ++$linelen;
        }
...
function rs_quoted_printable_encode_subject($string, $encoding = 'UTF-8')
    ...
    // L: so spam agents will not mark your email with QP_EXCESS.
...
if(!function_exists("highlightkeywords"))
    ...
    function highlightkeywords($text, $search, $partial_index = false, $field_name = "", $keywords_index = 1, $str_highlight_options = STR_HIGHLIGHT_SIMPLE)
        ...
        if(!isset($highlightkeywords) || ($highlightkeywords == false) || ($search == "") || ($text == ""))
            ...
            // A search is a small array so not much to lose by processing it.
            $hlkeycache = array();
            ...
            for($n = 0; $n < count($s); $n++)
                ...
                if(strpos($s[$n], ":") !== false)
                    ...
                    global $stemming;
                    // Stemming enabled, highlight any words matching the stem.
...
function str_highlight($text, $needle, $options = null, $highlight = null)
    ...
    License on the website reads: "All code on this website resides in the Public Domain, you are free to use and modify it however you wish."  http://aidanlister.com/repos/license/  */

    // Underscores are considered part of words, so temporarily replace them for better \b search.
...
// Returns all image sizes available.  Standard image sizes are $lang translated; custom sizes are i18n translated.
...
// Returns a user action log for $user. Standard field titles are $lang translated; custom titles are i18n translated.
...
// Given a comma separated user list (from the user select include file) turn all group entries into fully resolved list of usernames. Note that this function cannot decode default groupnames containing special characters.
function resolve_userlist_groups($userlist)
    ...
    $newlist = "";
    $ulist_count = count($ulist);

    for($n = 0; $n < $ulist_count; $n++)
        {
        $u = trim($ulist[$n]);
        if(strpos($u, $lang["group"] . ": ") === 0)
            {
            // Group entry, resolve.  Find the translated groupname.
            ...
            // Custom group, decode the groupname.
...
// Returns true if a password conforms to the configured paramaters or a descriptive string if it does not.
function check_password($password)
    ...
    for($m = 0; $m < strlen($password); $m++)
        ...
        else // Not alphanumeric, must be a special character.
            {
            ++$s;
            }
        }
...
// For a given keyword reference returns the related keywords.  Also reverses the process, returning keywords for
// matching related words and for matching related words and also returns other words related to the same keyword.
...
function send_statistics()
    ...
    if(time() - strtotime($last_sent_stats) < 604800) # 7*24*60*60
        {
        return false;
        }
...
// Returns a list of fields suitable for the simple search box.  Standard fields are $lang translated; custom fields are i18n translated.
function get_simple_search_fields()
    ...
    $return = array();
    $allfields_count = count($allfields);

    for($n = 0; $n < count($allfields); $n++)
        {
        // Check if for simple_search, also include the country field even if not selected.
        ...
        if(($allfields[$n]["simple_search"] == 1 || (isset($country_search) && $country_search && $allfields[$n]["ref"] == 3)) &&
...
function check_display_condition($n, array $field, array $fields, $render_js)
    ...
    // This field does not have a display condition, so it should be displayed.
    ...
    foreach($conditions as $condition)
        ...
        //  Check each field to see if needs to be checked.
        $display_check_data_count = count($display_check_data);
        for($cf = 0; $cf < $display_check_data_count; $cf++)
            {
            // Work out nodes submitted by user, if any.
            $ui_selected_node_values = array();

            if(isset($user_set_values[$display_check_data[$cf]['ref']])
            ...
            // Does the fieldname on this condition match the field being processed?  If so, field needs to be checked.
            if($s[0] == $display_check_data[$cf]['name'])
                {
                ...
                // Check display conditions, certain fixed list types allow for multiple nodes to be passed at the same time.
...
// Verify a supplied external access key with option to check keys.
function check_access_key($resource, $key)
    ...
    if(count($keys) == 0)
        ...
        // "Emulate" the user that emailed the resource by setting the same group and permissions        .
        $user = $keys[0]["user"];
        ...
        if(count($userinfo) > 0)
            ...
            else // Old style search filter that has not been migrated.
                {
            ...
            if($external_share_groups_config_options || stripos(trim(isset($userinfo[0]["config_options"])),"external_share_groups_config_options=true") !== false)
                ...
                // We need to get all globals as we do not know what may be referenced here.
                extract($GLOBALS, EXTR_REFS | EXTR_SKIP);
                eval($config_options);
                }

        // Special case for anonymous logins.  When a valid key is present, we need to log the user in as the
        // anonymous user so they will be able to browse the public links.
        global $anonymous_login;
        if(isset($anonymous_login))
            {
...
function check_access_key_collection($collection, $key)
    ...
    if($resources_count > 0)
        ...
        if($resources_count === count($invalid_resources))
            {
            return false;
            }
        }
...
// Returns a list of user groups selectable in the registration. Standard user groups are $lang translated; custom user groups are i18n translated.
...
// Return an array of indexed fields to which the current user does not have access.  Used by do_search to omit fields when searching.
...
// Returns a list of fields/properties with refs matching the supplied field refs, for search display setup.  This
// returns fewer columns and does not require that the fields be indexed, as in this case it is only used to judge
// whether the field should be highlighted.  Standard fields are $lang translated; custom fields are i18n translated.
...
function verify_extension($filename, $allowed_extensions = "")
    ...
    if($extension_count > 1)
        {
        $extension = trim(strtolower($extension[$extension_count - 1]));
        }
    else
        {
        return false;
...
/**
 * Detect if a path is relative or absolute.  If it is relative, we compute its absolute location by assuming it is
...
function getAbsolutePath($path, $create_if_not_exists = false)
    {
    // If the path start by a '/' or 'c:\', it is an absolute path.
    ...
    if($create_if_not_exists && !file_exists($folder))
        {
        mkdir($folder, 0777);
        }
    ...
    while(($file = readdir($directory_handle)) !== false)
        ...
        if(!in_array($file, array('.', '..')))
            ...
            if($include_hidden || ! preg_match('/^\./', $file))
                ...
                // If the path is a directory, and need to be explored.
...
/**
 * Returns filename component of path that is UTF-8 proof.
 * Thanks to nasretdinov at gmail dot com, @link http://www.php.net/manual/en/function.basename.php#85369
 *
 ...
 * Thanks to phparadise, http://fundisom.com/phparadise/php/file_handling/strip_file_extension
 *
...
/**
* Returns the path (relative to the gfx folder) of a suitable folder to represent a resource with the given resource
* type or extension.  Extension matches are tried first, followed by resource type matches.  Finally, if there are
* no matches then the 'type1' image will be used.  Set contactsheet to true to cd up one more level.
*
...
function get_nopreview_icon($resource_type, $extension, $col_size)
    ...
    // Legacy: Support the old location for resource type and GIF format (root of gfx folder).
...
// Returns true if a user account exists with email address $email.
...
function filesize_unlimited($path)
    ...
    // Bomb out, the output was nOt as we expected. Return the filesize() output.
    if(!is_int($bytesize))
        {
...
// String EnCrypt + DeCrypt function, Author: halojoy, July 2006
...
//  Returns an array with the resulting output (stdout and stderr).
function run_external($cmd, &$code)
    {
    // Thanks to dk at brightbyte dot de, http://php.net/manual/en/function.shell-exec.php
...
function sanitize_char($char)
    ...
    // Not a valid character, return an empty string.
    return "";
    }
...
// Formats a string with a collapsible more and less section.
                ...
                foreach($querylog as $query => $values)
                    ...
                    if(substr($query, 0, 7) != "explain" && $query != "show warnings")
                        ...
                        <?php if (strtolower(substr($query,0,6))=="select")
                            ...
                            $explain_count = count($explain);
                            for($n = 0; $n < $explain_count; $n++)
                                {
                                ?><tr><?php
...
/**
* Returns the full path to a utility, if installed or FALSE otherwise, but does not check if utility is working.
*
...
function get_utility_path($utilityname, &$checked_path = null)
    ...
    switch(strtolower($utilityname))
        ...
        case 'ghostscript':
            ...
            // Note that $check_exe is set to TRUE. In that way, get_utility_path() becomes backwards compatible
            // with the get_ghostscript_command().
...
/**
* Return an array of emails from a list of usernames and email addresses; with 'key_required' sibling array
* preserving the intent of internal and external sharing.
*
...
if(!function_exists('resolve_user_emails'))
    ...
    function resolve_user_emails($user_list)
        ...
        foreach($user_list as $user)
            ...
            // Internal, approved user account, add email address from user account.
            $emails_key_required['unames'][] = $user;
...
// Function to prevent cache arrays from going rogue, this will prevent long-running scripts from dying as these
...
function truncate_cache_arrays()
    ...
    if(count($GLOBALS['get_resource_data_cache']) > $cache_array_limit)
        {
        $GLOBALS['get_resource_data_cache'] = array();
        // Future improvement: get rid of only oldest, instead of clearing all? This would require a way to gauge the age of the entry.
        }
...
// Transforms txt in HTML, based on http://blog.matrixresources.com/blog/using-php-html-ize-plain-text.
function txt2html($txt)
    ...
    foreach($extArray as $ext => $replacement)
        ...
        // We have the pattern, the replacement, and the HTML being built; do the replacement.
...
function rs_setcookie($name, $value, $daysexpire = 0, $path = "", $domain = "", $secure = false, $httponly = true)
    ...
    if($daysexpire == 0)
        ...
        {
        $expire = time() + (86400 * $daysexpire); # 3600*24
        }
...
function get_resource_type_fields($restypes = "", $field_order_by = "ref", $field_sort = "asc", $find = "", $fieldtypes = array(), $include_inactive = false)
    ...
    $newfieldtypes = array_filter($fieldtypes, function($v)
        {
    ...
    // Allow for sorting, enabled for use by System Setup pages.      //if(!in_array($field_order_by,array("ref","name","tab_name","type","order_by","keywords_index","resource_type","display_field","required"))){$field_order_by="ref";}
    $allfields = sql_query("
        SELECT ref,
               name,
               title,
...
/**
* Utility function to generate URLs with query strings easier, with the ability to override existing query string
* parameters when needed.
*
* @param  string  $url
* @param  array   $parameters  Default query string params (e.g "k", which appears on most of ResourceSpace URLs).
* @param  array   $set_params  Override existing query string parameters.
*
* @return string
*/
...
function notify_resource_change($resource)
    ...
    $message_users = array();
    $download_users_count = count($download_users);
...
// Takes a string and add verbatim regex matches to the keywords list on found matches for that field.  It solves
// the problem, for example, indexing an entire "nnn.nnn.nnn" string value when '.' are used as a keyword separator.
// Uses config option $resource_field_verbatim_keyword_regex[resource type field] = '/regex/'  Also changes
// "field:<value>" type searches to "field:,<value>" for full matching for field types such as "Check box list"
// (config option to specify this).
function add_verbatim_keywords(&$keywords, $string, $resource_type_field, $called_from_search = false)
    ...
    if(!$called_from_search && isset($resource_field_checkbox_match_full) &&
...
// Tails a file using native PHP functions, first introduced with system console.
// Credit to: http://www.geekality.net/2011/05/28/php-tail-tackling-large-files
function tail($filename, $lines = 10, $buffer = 4096)
    {
    $f = fopen($filename, "rb"); #Open the file.
    fseek($f, -1, SEEK_END); # Jump to last character.

    // Read it and adjust line number if necessary; otherwise, the result would be wrong if file does not end with a blank line.
    if(fread($f, 1) != "\n") $lines -= 1;
...
function get_rs_session_id($create = false)
    ...
    if(isset($_COOKIE["rs_session"]))
        ...
        if(!headers_sent())
            {
            rs_setcookie("rs_session",$_COOKIE["rs_session"], 7, "", "", substr($baseurl, 0, 5) == "https", true);
            }
    ...
    if($create)
        ...
        if(count($valid) >= 1)
            {
            // Setup_user has not been called yet, we just need the usergroup.
...
/**
* Utility function used to move the element of one array from a position to another one in the same array.
...
// Add array_column if <PHP 5.5.                          <----- Is this function needed anymore?
...
// Returns an array of users (refs and emails) for use when sending email notifications (messages that in the past
// went to $email_notify, which can be emulated by using $email_notify_usergroups).  Can be passed a specific user
// type or an array of permissions.  Types supported: SYSTEM_ADMIN, RESOURCE_ACCESS, RESEARCH_ADMIN, USER_ADMIN,
// RESOURCE_ADMIN.
function get_notification_users($userpermission = "SYSTEM_ADMIN")
    ...
    else // An array has been passed, find all users with these permissions.
        {
        $condition = "";
...
function get_download_filename($ref, $size, $alternative, $ext)
    ...
    if($original_filenames_when_downloading)
        ...
        else // Fetch from field data or standard table.
            {
        ...
        if(strlen($origfile) > 0)
            ...
            // Use the original filename if one has been set, strip any path information (e.g. if the staticsync.php
...
function job_queue_add($type = "", $job_data = array(), $user = "", $time = "", $success_text = "", $failure_text = "", $job_code = "")
    ...
    // JSON_UNESCAPED_SLASHES is needed so we can effectively compare jobs.
    $job_data_json = json_encode($job_data, JSON_UNESCAPED_SLASHES);
...
function job_queue_get_jobs($type = "", $status = "", $user = "", $job_code = "", $job_order_by = "ref", $job_sort = "desc", $find = "")
    ...
    if($status != "")
        {
        $condition[] = " status ='" . escape_check($status) . "'";
        }
    if($user != "")
        {
    ...
    if($job_code != "")
        {
        $condition[] =" job_code ='" . escape_check($job_code) . "'";
        }
    if($find != "")
        {
        $find = escape_check($find);
...
/**
* Run offline job using defined job handler.
*
...
function job_queue_run_job($job, $clear_process_lock)
    ...
    // Variable used to avoid spinning off offline jobs from an already existing job.  Example: create_previews() is
    // using extract_text() and both can run offline.
...
/**
 * Generates a random string of requested length.  Used to generate initial spider and scramble keys.
 *
...
* @param integer $default                          The default value to use in case we cannot find it the mappings.
*
...
/**
* Helper function for Preview tools feature. Checks all necessary permissions or options in order to tell the system
* whether PreviewTools panel should be displayed.
*
...
function canSeePreviewTools($edit_access)
    ...
    return (($annotate_enabled && $edit_access) || $image_preview_zoom);
    }


/**
* Helper function for Preview tools feature. Checks if a config option that manipulates the preview image (on view
* page) is the only one enababled.
*
...
/**
* Determine if a video alternative was created from $ffmpeg_alternatives settings.  Places in this file because
* get_resource_path relies on it.
*
...
* @param integer $restype - Resource type that field applies to (0 = global).
...
*
* @return boolean =- TRUE if modal; otherwse, FALSE.
*/
function IsModal()
    {
    global $modal;
...
function generateCSRFToken($session_id, $form_id)
    ...
    $data = json_encode(array(
        "nonce" => generateSecureKey(128), # IMPORTANT: keep nonce at the beginning of the data array.
        "session" => $session_id,
        "timestamp" => time(),
        "form_id" => $form_id
    ));
...
* @uses rsDecrypt()
*
* @return boolean  Returns TRUE if token has been decrypted or CSRF is not enabled; otherwise, FALSE.
*/
...
function metadata_field_view_access($field)
    {
    return ((PHP_SAPI == 'cli' && !defined("RS_TEST_MODE"))
...
function check_share_password($key, $password, $cookie)
    ...
    // Set a cookie for this session so password will not be required again.
    if($cookie == "")
        {
...
function is_resourcespace_upgrade_available()
    ...
    if($centralised_version_number === false)
        ...
        ini_set('default_socket_timeout', 5); # Set timeout to 5 seconds in case server cannot access resourcespace.com
    ...
    elseif($product_version_data['major'] == $cvn_data['major']
...
*
* @param int $filterid  ID of filter (from usergroup search_filter_id or user search_filter_old).
*
* @return array
*/
function get_filter($filterid)
    {
    // Codes for filter 'condition' column: 1 = ALL must apply, 2 = NONE must apply, or 3 = ANY can apply.
...
function save_filter_rule($filter_rule, $filterid, $rule_data)
    ...
    $rule_data_count = count($rule_data);
    if($rule_data_count > 0)
        {
        $nodeinsert = array();
        for($n = 0; $n < $rule_data_count; $n++)
            {
            $condition = $rule_data[$n][0];
            for($rd = 0; $rd < $rule_data_count[$n][1]; $rd++)
                {
...
function has_browsebar()
    ...
    return isset($username) && !in_array($pagename, $not_authenticated_pages)
    && ('' == $k || $internal_share_access) && $browse_bar;
    // && false == $loginterms ?
    }
...
function check_script_last_ran($name, $fail_notify_allowance, &$last_ran_datetime)
    ...
    $script_failure_notify_seconds = intval($fail_notify_allowance) * 86400; # 24*60*60
    ...
    if('' != $script_last_ran)
        ...
        // It has been less than user allows it to last run, meaning it is all good.
...
/**
* Delete the specified metadata field, also deletes any node or resource_data rows associated with that field.
*
...
/*
 * Function to return a list of tab names retrieved from $fields array containing metadata fields.  If there is at
 * least one field with a value for tab_name, then if there is at another field that does not have a tab_name value, it
