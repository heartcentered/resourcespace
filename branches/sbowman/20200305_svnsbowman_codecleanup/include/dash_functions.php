<?php
// Homepage Dash Functions - Jethro, Montala Ltd
...
function update_dash_tile($tile, $url, $link, $title, $reload_interval, $all_users, $tile_audience, $current_specific_user_groups, $specific_user_groups, $default_order_by, $resource_count, $text = "", $delete = 1)
    ...
    // Sensible defaults for insertion into database.
    if(empty($reload_interval) || !is_numeric($reload_interval))
        {
        $reload_interval = 0;
        }
    ...
    if($tile_audience == 'true')
        ...
        if(count($current_specific_user_groups) > 0 || $tile["all_users"] == 0)
            {
            // Delete the users existing record to ensure they do not get a duplicate.
    ...
    elseif($tile_audience == 'specific_user_groups') // Specific usergroups tile.
        {
        // This is a usergroup specific dash tile. As not meant for a specific user group, remove it from the users immediately.
...
/*
 * Reorders the default dash, this is useful when you have just inserted a new tile or moved a tile and need to
 *   reorder them with the proper 10 gaps. Tiles should be ordered with values 10,20,30,40,50,60,70 for easy insertion.
 */
...
// Checks if an all_user tile is currently in use and therefore active for all_users. Pass the dash_tile.ref of tile to check.
...
// Cleanup duplicate and loose tiles. This removes all unused tiles that are flagged as "allowed to delete".
...
// Checks the configuration for each custom tile. If the config for the tile is still there, then return true.
function checkConfigCustomHomePanels($tile, $tile_style)
    {
    global $custom_home_panels;

    $tile_config_set = false;
    $custom_home_panels_count = count($custom_home_panels);

    for($n = 0; $n < $custom_home_panels_count; $n++)
        {
    ...
    foreach($tiles as $tile)
        ...
        >
            <div id="contents_tile<?php echo htmlspecialchars($tile["tile"]);?>" class="HomePanelIN HomePanelDynamicDash <?php echo $contents_tile_class; ?> <?php echo ($dash_tile_shadows)? "TileContentShadow":"";?>" style="<?php echo $tile_custom_style; ?>">
                <?php
                // Only pre-render the title if using a "standard" tile and we know the H2 will be in the target data.
...
function get_managed_dash()
    {
    global $baseurl, $baseurl_short, $lang, $anonymous_login, $username, $dash_tile_shadows, $anonymous_default_dash, $userref, $usergroup, $dash_tile_colour, $dash_tile_colour_options, $managed_home_dash, $help_modal;
...
/*
 * Get user_dash_tile record, provide the user_dash_tile ref as the $tile.
 ...
 */
  ...
  * The Add_user_dash_tile function used checks for an existing match so that it will not duplicate tiles on a users dash.
  */
...
/*
 * Reorders the users dash, this is useful when you have just inserted a new tile or moved a tile and need to
 *  reorder them with the proper 10 gaps. Tiles should be ordered with values 10,20,30,40,50,60,70 for easy insertion.
 */
function reorder_user_dash($user)
    ...
    $user_tiles_count = count($user_tiles);
    $order_by = 10 * $user_tiles_count;

    $sql = "UPDATE user_dash_tile SET order_by = (CASE ";
    for($i = $user_tiles_count - 1; $i >= 0; $i--)
        {
...
function build_dash_tile_list($dtiles_available)
    ...
    foreach($dtiles_available as $tile)
        ...
        <tr id="tile<?php echo $tile["ref"];?>" <?php if(isset($buildstring["tltype"]) && $buildstring["tltype"]=="conf") {echo "class=\"conftile\"";} ?>>
            ...
            <td>
                <?php
                if($tile["allow_delete"] && (($tile["all_users"] && checkPermission_dashadmin()) ||
...
* At the moment it is only available for blank search tiles and text text only tiles.
*
