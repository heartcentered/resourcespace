...
// Indicate that an intermediate JPG is being made, so that image_processing does not skip the HPR.
global $keep_for_hpr;
...
else // Use temporary preview source/destination, user has uploaded a file intended to replace the previews only.
    {
...
// QuickLook Previews (Mac only) for everything except Audio/Video files, attempt to generate a QuickLook preview first.
...
// Try Photoshop Thumbnail.  For good results, Photoshop Preferences must be set to save Preview image at Extra Large size.
...
// Photoshop Transparency Checkerboard for PSD transparency. Not applicable to $photoshop_thumb_extract.
...
if(($extension == "cr2" || $extension == "nef" || $extension == "dng" || $extension == "raf" || $extension == "rw2" || 'arw' == $extension) && !isset($newfile))
    {
    global $cr2_thumb_extract, $nef_thumb_extract, $dng_thumb_extract, $rw2_thumb_extract, $raf_thumb_extract, $arw_thumb_extract;

    if(($extension == "cr2" && $cr2_thumb_extract) || ($extension == "nef" && $nef_thumb_extract) ||
        ($extension == "dng" && $dng_thumb_extract) || ($extension == "rw2" && $rw2_thumb_extract) ||
        ($extension == "raf" && $raf_thumb_extract) || ('arw' == $extension && $arw_thumb_extract))
        {
        if($exiftool_fullpath != false)
            {
            ...
            // NOTE: in case of failures, other suboptimal possibilities may be explored in the future, such as
            // -thumbnailimage and -jpgfromraw, like this:
            // if (!filesize_unlimited($target)>0)
                // {
                // unlink($target);
...
/* Unoconv is a python-based utility to run files through OpenOffice. It is available in Ubuntu.  This adds
 * conversion of office docs to PDF format and adds them as alternative files (this behaviour can be disabled by
 * adding the extension to non_image_types list).  One could also see the potential to base previews on the PDFs for
 * paging and better quality for most of these formats.
*/
...
if(in_array($extension,$unoconv_extensions) && $extension != 'pdf' && isset($unoconv_path) && !isset($newfile))
    ...
    $no_alt_condition = ($GLOBALS['non_image_types_generate_preview_only']
        && in_array($extension, $GLOBALS['non_image_types']) && in_array($extension, config_merge_non_image_types()));
    ...
    elseif(file_exists($pdffile)) // Attach this PDF file as an alternative download.
        {
...
global $calibre_extensions, $calibre_path;
...
// Try Microsoft OfficeOpenXML Format  Also try Micrsoft XPS... the sample document I've seen uses the same path for
// the preview, so it will likely work in most cases, but I think the specs allow it to go anywhere.
...
if($extension == "blend" && !isset($newfile))
    ...
    if(!file_exists($blendercommand) || is_dir($blendercommand))
        {
        $blendercommand = $blender_path . "/blender";
        }
    if(!file_exists($blendercommand))
        {
        $blendercommand = $blender_path . "\blender.exe";
        }
    if(!file_exists($blendercommand))
        {
...
// Microsoft Word previews using Antiword, note: this is very basic.
if($extension == "doc" && isset($antiword_path) && isset($ghostscript_path) && !isset($newfile))
    ...
    if(file_exists($target . ".ps"))
        ...
        if(file_exists($target))
            {
            // A JPG was created, set as the file to process.
            $newfile = $target;
            }
        }
    }
...
if(($extension == "mp3" || $extension == "flac") && !isset($newfile))
    ...
    // If the file contains an image, use it; if it is blank, it needs to be erased because it will cause an error in ffmpeg_processing.php
    if(file_exists($target))
        {
        if(filesize_unlimited($target) > 0)
            {
...
// Try text file to JPG conversion, support text files simply by rendering them on a JPG.
...
elseif(($ffmpeg_fullpath != false) && !isset($newfile) && in_array($extension, $ffmpeg_supported_extensions))
    ...
    if(preg_match('/Duration: (\d+):(\d+):(\d+)\.\d+, start/', $out, $match))
        ...
        // Generate snapshots for the whole video (not for alternatives).  Custom target used ONLY for captured
        // snapshots during the video.
        if(1 < $ffmpeg_snapshot_frames && -1 == $alternative)
            ...
            elseif($video_resolution['width'] < $video_resolution['height'] && isset($snapshot_height) && $video_resolution['height'] >= $snapshot_height) // Portrait.
                {
...
if((!isset($newfile)) && (!in_array($extension, $ffmpeg_audio_extensions)) && (!in_array($extension, $ffmpeg_supported_extensions)))
    ...
    // For EPS/PS/PDF files, use GhostScript directly and allow multiple pages.
    if(($extension == "pdf") || (($extension == "eps") && !$photoshop_eps) || ($extension == "ai") || ($extension == "ps"))
        ...
        // Since this is not an application required size we cannot assume there is a record for it.
        if(empty($scr_size))
            {
        ...
        /* We want to rip at ~150 dpi by default because it provides decent quality previews and speed in the end. It
         * is not always efficient to just rip at 150, though, because for very large pages, a lot of pixels get
         * wasted when we resize to 850 pixels. Also, if the page size is quite small, ripping at 150 may not provide
         * enough quality for the SCR size preview. So, use PDFinfo to calculate a rip resolution that will give us a
         * source bitmap of approximately 1600 pixels.
        */
        if($pdf_dynamic_rip)
            {
            if($extension == "pdf")
        ...
        for($n = 1; $n <= $pdf_pages; $n++)
            ...
            // Use screen size for other pages.
            if($n > 1)
                {
                $size = "scr";
                }
            ...
            if(file_exists($target) && $pdf_split_pages_to_resources)
                ...
                $pdf_split_pages_to_resources = false; # So we do not get stuck in a loop creating split pages for the single page PDFs.
