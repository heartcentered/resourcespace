...
// Run report with ID $ref for the date range specified and returns a result array.
        ...
        for($n = 0; $n < $count_results; $n++)
            ...
            foreach ($result as $key => $value)
                ...
                elseif($key != "thumbnail")
                    ...
                    // Remove comma prefix on dropdown and checkbox values.
                    if(substr($value, 0, 1) == ",")
                        {
...
function send_periodic_report_emails($echo_out = true)
    ...
    // Query to return all 'pending' report e-mails, i.e. where we have not sent one before OR one is now overdue.
    $query = "
        SELECT pe.*,
               u.email,
               r.name
    ...
    foreach($reports as $report)
        {
        $start = time() - (86400 * $report["period"]); # 60*60*24
        ...
        // Send mail to original user, this contains the unsubscribe link. Note: this is basically the only way at the
        // moment to delete a periodic report.
        ...
        // Send to all other active users, if configured.  Send the report to all active users.
...
// Activity types are stored in plain text english in daily_stat. This function will use language strings to resolve a
// translated value where one is set.
