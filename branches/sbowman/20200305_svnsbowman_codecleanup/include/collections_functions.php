...
if (!function_exists("get_user_collections"))
    ...
    function get_user_collections($user, $find = "", $order_by = "name", $sort = "ASC", $fetchrows = -1, $auto_create = true)
        ...
        elseif(strlen($find) == 1 && !is_numeric($find)) # A-Z search.
            {
        ...
        // Include themes in my collecions?  Only filter out themes if $themes_in_my_collections is set to false in config.php.
        ...
        // User has no collections of their own, and this is not a search, so make a new 'Default Collection'.
...
function add_resource_to_collection($resource, $collection, $smartadd = false, $size = "", $addtype = "")
    ...
    // Cannot always block adding resource types, since this may be a single resource managed request.
    if($addpermitted && !$smartadd && (count($collection_block_restypes)>0))
        {
        if($addtype == "")
            {
    ...
    // Check if this collection has already been shared externally. If it has, we must fail if not permitted or add a further entry for this specific resource, and warn the user that this has happened.
    if($addpermitted)
        {
        ...
        if($keys_count > 0)
            ...
            // Check if user can share externally and has open access. We should not add this if they cannot share externally, have restricted access or only been granted access.
            if(!can_share_resource($resource))
                {
                return false;
                }

            # Set the flag so a warning appears and check to see if all shares have expired.
...
function collection_writeable($collection)
    ...
    if($allow_smart_collections && !isset($userref))
        {
        // So "you cannot modify this collection".
...
function collection_readable($collection)
    ...
    // Access if collection_commenting is enabled and request feedback checked, if it is a public collection or theme, or if k is not empty or option to ignore collection access is enabled and k is empty.
    global $userref, $usergroup, $ignore_collection_access, $collection_commenting;
    if($collection_commenting && $collectiondata['request_feedback'] == 1 || $collectiondata["public"] == 1 ||
    getval("k", "") != "" || getval("k", "") == "" && $ignore_collection_access)
        {
        return true;
        }
    ...
    if(is_numeric($userref))
        {
        // Access if: it is their collection, it is a public collection or theme, they have the 'access and edit all collections' admin permission, they are attached to this collection.
        // Option to ignore collection access is enabled and k is empty.
        if($userref == $collectiondata["user"] || $collectiondata["public"] == 1 || checkperm("h") ||
        in_array($userref, $attached)  || in_array($usergroup,$attached_groups)|| /*(checkperm("R") && $request) ||*/
        getval("k", "") != "" || (getval("k", "") == "" && $ignore_collection_access))
            {
            return true;
            }
...
if(!function_exists("create_collection"))
    {
    function create_collection($userid, $name, $allowchanges = 0, $cant_delete = 0, $ref = 0, $public = false, $categories
    = array())
        {
...
// Performs a search for themes / public collections, returns a comma separated list of resource refs in each collection, used for thumbnail previews.
if(!function_exists("search_public_collections"))
    {
    function search_public_collections($search = "", $order_by = "name", $sort = "ASC", $exclude_themes = true,
    $exclude_public = false, $include_resources = false, $override_group_restrict = false, $search_user_collections = false)
        {
        global $userref;
        $sql = "";
        $keysql = "";
        ...
        // Include only public collections.
        if($exclude_themes)
            {
            $sql .= " and (length(c.theme)=0 or c.theme is null)";
            }

        // Exclude public only collections, return only themes.
        if(($exclude_public) && !$search_user_collections)
            {
        ...
        if($include_resources)
            {
            return sql_query("SELECT distinct c.*,u.username,u.fullname, count( DISTINCT cr.resource ) count FROM
            collection c left join collection_resource cr ON c.ref=cr.collection left outer join user u ON c.user=u.ref
            left outer join collection_keyword k ON c.ref=k.collection $keysql WHERE $sql_public $sql GROUP BY c.ref ORDER
            BY " . escape_check($order_by) . " " . escape_check($sort));
            }
        else
            {
            return sql_query("SELECT distinct c.*,u.username,u.fullname FROM collection c left outer join user u ON
            c.user=u.ref left outer join collection_keyword k ON c.ref=k.collection $keysql WHERE $sql_public $sql GROUP BY
...
function do_collections_search($search, $restypes, $archive = 0, $order_by = '', $sort = "DESC")
    ...
    if($search_includes_themes_now || $search_includes_public_collections_now || $search_includes_user_collections_now)
        ...
        if($collection_search_includes_resource_metadata)
            ...
            $collections = search_public_collections($search, "theme", "ASC", !$search_includes_themes_now,
            !$search_includes_public_collections_now, true, false, $search_includes_user_collections_now);
            }
...
function add_collection($user, $collection)
    {
    // Do not add if we are anonymous, we can only have one collection.
    global $anonymous_login, $username, $anonymous_user_session_collection;
    if(isset($anonymous_login) && ($username == $anonymous_login) && $anonymous_user_session_collection)
        {
        return false;
...
if(!function_exists("index_collection"))
    ...
    function index_collection($ref, $index_string = '')
        {
        // Remove existing keywords and define an indexable string from the name, themes, and keywords.
        sql_query("DELETE FROM collection_keyword WHERE collection='" . escape_check($ref) . "'");
        ...
        // If an index string was not supplied, generate one.
        if(!strlen($index_string) > 0)
            ...
            $indexarray_count = count($indexarray);
            for ($i = 0; $i < $indexarray_count; $i++)
                {
...
function save_collection($ref, $coldata = array())
    ...
    if(($users) != "")
        ...
        if(!empty($new_attached_users))
            ...
            message_add($new_attached_users, str_replace(array('%user%', '%colname%'), array($collection_owner,
            getvalescaped("name", "")), $lang['collectionprivate_attachedusermessage']), $baseurl . "/?c=" . $ref);
            }
        }
...
// Return a list of theme headers, i.e. theme categories and return sql_array("select theme value,count(*) c from collection where public=1 and length(theme)>0 group by theme order by theme");
...
function get_theme_headers($themes = array())
    ...
    for($x = 0; $x < $themes_count; $x++)
        ...
        elseif (isset($themes[$x])&& $themes[$x] != ""&& $x != 0)
            {
    ...
    for($n = 0; $n < $themes_count; $n++)
        {
        if((!in_array($themes[$n][$selecting],$return)) && # De-duplicate as there are multiple collections per theme category
...
if(!function_exists("get_themes"))
    ...
    function get_themes($themes = array(""), $subthemes = false)
        ...
        if($themes_order_by != "name")
            {
            $order_sort = " order by $themes_order_by $sort";
            }
        ...
        if($themes_order_by == "name")
            ...
            elseif($sort == "DESC")
                {
                usort($collections, 'collections_comparator_desc');
                }
            }
        ...
        if($themes_with_resources_only)
            ...
            $collections_orig_count = count($collections_orig);
            $collections = array();

            for($i = 0; $i < $collections_orig_count; $i++)
                {
...
if(!function_exists("email_collection"))
    {
    function email_collection($colrefs, $collectionname, $fromusername, $userlist, $message, $feedback, $access = -1,
    $expires = "",$useremail = "", $from_name = "", $cc = "", $themeshare = false, $themename = "", $themeurlsuffix = "",
    $list_recipients = false, $add_internal_access = false, $group = "", $sharepwd = "")
        {
        // Attempt to resolve all users in the string $userlist to user references, add $collection to these users My collections page, send them an e-mail linking to this collection, and handle multiple collections (comma separated list).
        ...
        $urefs_count = count($urefs);
            ...
            // Insert new user_collection row(s) and loop through the collections.
            for($nx1 = 0; $nx1 < $reflist_count; $nx1++)
                {
                // Loop through the users.
                for($nx2 = 0; $nx2 < $urefs_count; $nx2++)
                    {
        ...
        // Send an email to each resolved user, htmlbreak is for composing list.
        $htmlbreak = "\r\n";
        global $use_phpmailer;

        if($use_phpmailer)
            {
            $htmlbreak = "<br /><br />";$htmlbreaksingle="<br />";
            }
        ...
        // Change the text if sharing a theme category.
        if($themeshare)
            {
        ...
        for($nx1 = 0; $nx1 < $emails_count; $nx1++)
            {
            ...
            // Do not send a whole list of collections if internal, just send the theme category URL.
            if($themeshare && !$key_required[$nx1])
                ...
                // Log this.
                for($nx2 = 0; $nx2 < $reflist_count; $nx2++)
                    {
                ...
                for($nx2 = 0; $nx2 < $reflist_count; $nx2++)
                    ...
                    // Do we need to add an external access key for this user (specified email rather than username)?
                    if($key_required[$nx1])
                        {
...
// For naming smart collections, takes a full searchstring with the form 'search=restypes=archive=starsearch=' (all parameters optional) and uses search_title_processing to autocreate a more informative title.
...
function add_saved_search_items($collection, $search = "", $restypes = "", $archivesearch = "", $order_by = "relevance", $sort = "desc", $daylimit = "", $starsearch = "")
    ...
    // Check if this collection has already been shared externally. If it has, we must add a further entry for this specific resource, and warn the user that this has happened.
    $keys = get_collection_external_access($collection);
    $keys_count = count($keys);
    ...
    $blockedtypes_count = count($blockedtypes);

    if(!empty($resourcesnotadded) || $blockedtypes_count > 0)
        {
...
function get_theme_image($themes = array(), $collection = "", $smart = false)
    {
    global $theme_images_number, $theme_category_levels, $usergroup, $userref, $userpermissions;
    ...
    if($images_override !== false && is_array($images_override))
        ...
        if($smart)
            ...
            $sqlfilter = " WHERE c.public=1 and c.theme='" . escape_check($themes[0]) . "' " . $sqlfilter_custom;
...
function save_collection_resource_comment($resource, $collection, $comment, $rating)
    ...
    if($comment != $data[0]['comment'])
        {
        collection_log($collection, "m", $resource);
        }
    if($rating != $data[0]['rating'])
        {
        collection_log($collection, "*", $resource);
        }
    return true;
    }
...
function send_collection_feedback($collection, $comment)
    ...
    if($feedback_resource_select)
        ...
        // Append Adobe Lightroom compatible summary.
    ...
    // Always send an email for the feedback whatever the user preference, since the feedback may be very long so can then refer to the CC'd email.
    ...
    if(!$send_email)
        {
        global $userref;
        message_add($user["ref"], $lang["collectionfeedback"] . " - " . $cinfo["name"] . "<br />" . $body, "", (isset($userref)) ? $userref : $user['ref'], MESSAGE_ENUM_NOTIFICATION_TYPE_SCREEN, 2592000); # 60*60*24*30
        }
...
// Generates an HTML link for adding a changing the current collection.
...
function collection_log($collection, $type, $resource, $notes = "")
    ...
    if($modifiedcollogtype)
        {
        $type = $modifiedcollogtype;
        }

    $modifiedcollognotes = hook("modifycollognotes", "", array($type, $resource, $notes));
    if($modifiedcollognotes)
        {
        $notes = $modifiedcollognotes;
        }
    ...
    sql_query("INSERT INTO collection_log (date, user, collection, type, resource, notes)
...
function collection_set_themes ($collection, $categories = array())
    ...
    if(!is_numeric($collection) || !is_array($categories) || $categories_count > $theme_category_levels)
        {
        return false;
        }
    $sql = "update collection set public = 1";

    for($n = 0;$n < $categories_count; $n++)
        {
        if($n == 0)
            {
            $categoryindex = "";
...
function show_hide_collection($colref, $show = true, $user = "")
    ...
    else // Get hidden collections for user.
        {
...
if(!function_exists("compile_collection_actions"))
    ...
    function compile_collection_actions(array $collection_data, $top_actions, $resource_data = array())
        {
        global $baseurl_short, $lang, $k, $userrequestmode, $zipcommand, $collection_download, $use_zip_extension, $archiver_path, $manage_collections_contact_sheet_link, $manage_collections_share_link, $allow_share,
        ...
        // Collection bar actions should always be a special search !collection[ID] (exceptions might arise but most
        // of the time it should be handled using the special search). If top actions then search may include
        // additional refinement inside the collection.
        ...
        // View all resources.
        if(!$top_actions // View all resources makes sense only from collection bar context.
            && (($k == "" || $internal_share_access) && (isset($collection_data["c"]) && $collection_data["c"] > 0)
        ...
        if(((checkperm('c') || checkperm('d')) && $collection_data['savedsearch'] == 0
            && ($userref == $collection_data['user'] || $collection_data['allow_changes'] == 1 || checkperm('h')))
        ...
        if($pagename != 'collections' && ($k == '' || $internal_share_access) && !checkperm('b')
            ...
            && ((isset($search_collection) && isset($usercollection) && $search_collection != $usercollection) || !isset($search_collection)) && collection_readable($collection_data['ref']))
            {
        ...
        // Edit All, if this collection is fully editable, then display an Edit All link.
        ...
        // Add option to publish as a Featured Collection.
        ...
        // Remove.
        if(($k == "" || $internal_share_access) && $manage_collections_remove_link && $userref != $collection_data['user']
            && !checkperm('b') && collection_readable($collection_data['ref']))
            {
        ...
        // Delete All, Note: functionality moved from edit collection page.
        if(($k == "" || $internal_share_access) && !$top_actions
            && ((is_array($result) && count($result) != 0) || $count_result != 0)
            && (isset($allow_resource_deletion) && $allow_resource_deletion)
            && collection_writeable($collection_data['ref']) && $allow_multi_edit && !checkperm('D'))
            {
        ...
        if(0 < $count_result && !$top_actions && ($k =='' || $internal_share_access)
            ...
            // Hide Collection.
            $user_mycollection = sql_value("sSELECT ref value FROM collection WHERE user='" . escape_check($userref) . "' AND name='Default Collection' ORDER BY ref limit 1","");

            // Check that this collection is not hidden; otherwise, use first in alphabetical order.
...
function new_featured_collection_form(array $themearray = array())
    ...
    <div class="BasicsBox">
        ...
        // Root level does not allow collections so the only option for the user is to just create a featured collection category level.
        if(0 === $themes_count)
            { ?>
            <input type="hidden" name="location" value="subfolder"> <?php
            }
...
function collection_download_use_original_filenames_when_downloading(&$filename, $ref, $collection_download_tar, &$filenames, $id = '')
    ...
    /*
     * Only perform the copy if an original filename is set. Now you have got original filename, but it may have an
     * extension in a different letter case. The system needs to replace the extension to change it to JPG if
     * necessary, but if the original file is being downloaded, and it originally used a different case , then it
     * should not come from the file_extension, but rather from the original filename itself.
     * Do an extra check to see if the original filename might have uppercase extension that can be preserved.
     * Also, set extension to "" if the original filename didn't have an extension (exiftool identification of
     * filetypes).
    */
    ...
    // Copy to tmp if Exiftool failed or rename this file for extra efficiency to reduce copying and disk usage.
...
function collection_download_process_collection_download_name(&$filename, $collection, $size, $suffix, array $collectiondata)
    ...
    if(empty($filename))
        ...
        else // Default: Do not include the collection name in the filename.
            {
...
function relate_all_collection($collection, $checkperms = true)
    ...
    for($n = 0; $n < $rlist_count; $n++)
        {
        for($m = 0; $m < $rlist_count; $m++)
            {
            if($rlist[$n] != $rlist[$m]) # Do not relate a resource to itself.
                {
