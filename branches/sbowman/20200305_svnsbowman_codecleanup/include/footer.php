...
// Do not display header or footer when dynamically loading CentralSpace contents.
if(getval("ajax", "") == "" && !hook("replace_footer"))
    ...
    if(!$preview_header_footer)
        {
        $omit_footer_pages[] = "preview";
        }
                ...
                if($bottom_links_bar && ($about_link || $contact_link))
                    ...
                    if(!hook("replacefooterlinks"))
                        ...
                        if(!$use_theme_as_home && !$use_recent_as_home)
                            {
                            ?><li><a href="<?php echo $baseurl?>/pages/<?php echo $default_home_page?>" onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["home"]?></a></li><?php
                            }
                        if($about_link)
                            {
                            ?><li id="footer_about_link"><a href="<?php echo $baseurl?>/pages/about.php" onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["aboutus"]?></a></li><?php
                            }
                        if($contact_link || $nav2contact_link)
                            {
                            ?><li><a href="<?php echo $baseurl?>/pages/contact.php" onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["contactus"]?></a></li><?php
                            }
...
if($show_resource_title_in_titlebar)
    ...
    // Clear resource or search title for pages that do not apply.
    ...
    elseif(in_array($pagename, $search_title_pages))
        ...
        elseif(($pagename == "collection_download") || $pagename == "edit" && getval("collection", "") != "")
            {
...
if(getval("ajax", "") == "")
    {
    // Do not show closing tags if we are in AJAX mode.
    echo "<!--CollectionDiv-->";
