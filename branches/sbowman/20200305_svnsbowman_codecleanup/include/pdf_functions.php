...
function get_pdf_template_path($resource_type, $template_name = '')
    ...
    // Client code wants a specific template name, but there is not one.
...
/**
* Takes an HTML template suitable for the HTML2PDF library and generates a PDF file if successful.
*
...
* @param  array    $bind_placeholders   A map of all the values that are meant to replace any placeholders found in
*                                        the HTML template.
...
function process_if_statements($original_string, array $bind_params)
    ...
    foreach($if_isset_matches[0] as $if_isset_match)
        ...
        /*
        Make sure we have the correct subset (html2) for our if statement. This means we do not stop at the first endif we found unless there are no if statements inside the subset.  If there are, move passed it and continue looking for other endifs until we reach the same number of endifs as we had ifs.
        */
        $endif_count = 0;
        do
            {
...
/**
* Function to convert the user's language into an HTML2PDF supported language.  Scans the HTML2PDF locale folder to
* create a list of supported languages to compare against the set user language. Also resolves dialects when possible.
* Fallback set to 'en'.
*
* @return string
*/
