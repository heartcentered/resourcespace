...
function do_search($search, $restypes = '', $order_by = 'relevance', $archive = '0', $fetchrows = -1, $sort = 'desc',
    $access_override = false, $starsearch = 0, $ignore_filters = false, $return_disk_usage = false,
    $recent_search_daylimit = '', $go = false, $stats_logging = true, $return_refs_only = false, $editable_only = false,
    $returnsql = false)
    {
    ...
    // Default to ascending if not a valid "DESC".
    ...
    if(!in_array($order_by, $order) && (substr($order_by, 0, 5) == "field"))
        ...
        // Establish sort order (numeric or otherwise).  Attach ref as a final key to foster stable result sets which
        // should eliminate resequencing when moving <- and -> through resources (in view.php).
    ...
    // Allows for searching in more than one archive state.
    $archive = explode(",", $archive);

    hook("modifyorderarray");

    // IMPORTANT! Add to this array in the format [AND group]=array(<list of nodes> to OR).
    $node_bucket = array();
    ...
    // Fail safe by falling back to default if not found.
    $order_by = (isset($order[$order_by]) ? $order[$order_by] : (substr($search, 0, 11) == '!collection' ? $order['collection'] : $order['relevance']));
    ...
    if(substr($search, 0, 1) == "!" && substr($search, 0, 6) != "!empty")
        ...
        else // Extract search params.
            {
            $search_params = substr($search, $s + 1);
            }
        }
    ...
    // One extra join (rca2) is required for user specific permissions (enabling more intelligent watermarks in search
    // view) the original join is used to gather group access into the search query as well.
    if((!checkperm("v")) && !$access_override)
        ...
        // If rca.resource is null, then no matching custom access record was found.  If r.access is also 3 (custom)
        // then the user is not allowed access to this resource. Note that it is normal for null to be returned if
        // this is a resource with non custom permissions (r.access<>3).
        if($sql_filter != "")
            {
            $sql_filter .= " AND ";
            }
    ...
    // Modify select hook 1.
    $modified_select = hook('modifyselect');
    $select .= $modified_select ? $modified_select : '';

    // Modify select hook 2.
    $modified_select2 = hook('modifyselect2');
    $select .= $modified_select2 ? $modified_select2 : '';

    // Disk usage.
    $select .= $return_disk_usage ? ',r.disk_usage' : '';

    // Select group and user access rights if available; otherwise, select null values so columns can still be used
    // regardless.  This makes group and user specific access available in the basic search query, which can then be
    // passed through access functions in order to eliminate many single queries.
    if(!checkperm("v") && !$access_override)
        {
    ...
    if($keysearch)
        ...
        for($n = 0; $n < $keywords_count; $n++)
            ...
            // If quoted string with a field specified, we first need to try and resolve it to a node instead of working out keyword positions, etc.
            if(!$quoted_string || ($quoted_string && strpos($keyword, ":") !== false))
                {
                if(substr($keyword, 0, 1) != "!" || substr($keyword, 0, 6) == "!empty")
                    ...
                    elseif($field_short_name_specified && !$ignore_filters && isset($fieldinfo['type']) && in_array($fieldinfo['type'], $FIXED_LIST_FIELD_TYPES))
                        {
                        // We have searched using a legacy format (ie. fieldShortName:keyword), try and convert it to @@NodeID.
                        ...
                        foreach($keywords_expanded as $keyword_expanded)
                            ...
                            if(false !== $field_node_index)
                                ...
                                // String has been resolved to a node so even if it is quoted we do not need to process it as a quoted string now.
                                $quoted_field_match = true;
                                $keywordprocessed = true;
                                }
                            }
                        }

                    // Need this also for string matching in a named text field.
                    ...
                    // Need this also for string matching in a named text field.
                    if(!$quoted_string && !$keywordprocessed && !($field_short_name_specified && hook('customsearchkeywordfilter', null, array($kw))))
                        {
                        // Normal keyword, searches all fields that the user has access to. If ignoring field
                        // specifications, then remove them.
                        ...
                        // Sskip common words that are excluded from indexing.
                        if(in_array($keyword, $noadd))
                            {
                            $skipped_last = true;
                            debug("do_search(): skipped common word: {$keyword}");
                            }
                        else  // Handle Wildcards
                            {
                            $wildcards = array();
                            ...
                            // Resolve keyword and ignore any wildcards. We need wildcards to be present later, but not here.
                            $keyref = resolve_keyword(str_replace('*', '', $keyword), false, true, !$quoted_string);
                            ...
                            if($keyref === false && !$omit && !$empty && count($wildcards) == 0 && !$field_short_name_specified)
                                ...
                                else // No keyword match, but there is a word that sounds like this word, suggest this word instead.
                                    {
                                    $suggested[$n] = "<i>" . $soundex . "</i>";
                                    }
                                }
                            else // Found Wildcards
                                {
                                ...
                                if($omit)
                                    ...
                                    // We are dealing with a special search checking if a field is empty.
                                    if($empty)
                                        {
                                        ...
                                        else // Check that resource data is empty.
                                            {
            ...
            if($quoted_string && !$quoted_field_match)
                ...
                // This keyword is a quoted string, split into keywords but do not preserve quotes this time.
                ...
                // Set the counter to the first keyword.
                $qk = 1;
                ...
                foreach($quotedkeywords as $quotedkeyword)
                    ...
                    // Skip common words that are excluded from indexing.
                    if(in_array($quotedkeyword, $noadd))
                        {
                        $skipped_last = true;
                        }
                    else
                        ...
                        // Support skipped keywords, if the last keyword was skipped (listed in $noadd), increase the
                        // allowed position from the previous keyword. Useful for quoted searches that contain $noadd
                        // words, e.g. "black and white" where "and" is a skipped keyword.
                        ...
                        // Resolve keyword.
                        $keyref = resolve_keyword($quotedkeyword, false, true, false);
                        if($keyref === false)
                            {
                            // Make a new keyword.
                ...
                if($omit)
                    ...
                    // Instead of adding to the union, filter out resources that do contain the quoted string.
                    $sql_filter .= str_replace("[bit_or_condition]",""," r.ref NOT IN (SELECT resource FROM (" . $freeunion .  " WHERE " . $freeunioncondition . " GROUP BY resource UNION " .  $fixedunion . " WHERE " . $fixedunioncondition . ") qfilter[union_index]) ");
                    }
                else
                    {
    ...
    if($fullmatch == false)
        ...
        else // Suggest alternative spellings/sound-a-likes.
            {
            $suggest = "";
        ...
        for($n = 0; $n < $sf_count; $n++)
            ...
            if(!$filter_not)
                ...
                if(count($ff) > 0)
                    ...
                    // Oonly for those without 'v' (which grants access to all resources).
                ...
                if(count($fn) > 0)
                    ...
                    // Only for those without 'v' (which grants access to all resources).
                    if(!checkperm("v") && !$access_override && $custom_access_overrides_search_filter)
                        {
                        $sql_join .= "or ((rca.access IS NOT null AND rca.access<>2) or (rca2.access IS NOT null AND rca2.access<>2))";
                        }
                ...
                if(count($ff) > 0)
                    ...
                    $sql_filter .= "((r.ref NOT IN (SELECT resource FROM resource_keyword WHERE resource_type_field IN ('" . join("','", $ff) . "') AND keyword IN ('" .    join("','", $kw) . "'))) ";
                    }
                ...
                if(count($fn) > 0)
                    ...
                    // Filter out resources that do contain the keyword(s).
                    $sql_filter .= "((r.ref NOT IN (SELECT rn.resource FROM resource_node rn LEFT JOIN node n ON rn.node=n.ref WHERE n.resource_type_field IN ('" . join("','", $fn) . "') AND n.name IN ('" .    join("','", $ks) . "'))) ";
                    }

                // Option for custom access to override search filters.  For this resource, if custom access has been
                // granted for the user or group, nullify the search filter for this particular resource effectively
                // selecting "true".
                global $custom_access_overrides_search_filter;
    ...
    // Keyword union assembly.  Use UNIONs for keyword matching instead of the older JOIN technique, much faster.
    // Assemble the new join from the stored unions.
    $sql_keyword_union_count = count($sql_keyword_union);
    if($sql_keyword_union_count > 0)
        {
        for($i = 1; $i <= $sql_keyword_union_count; $i++)
            {
            $bit_or_condition = "";
            for($y = 1; $y <= $sql_keyword_union_count; $y++)
                {
                if($i == $y)
                    {
        ...
        $sql_keyword_union_criteria_count = count($sql_keyword_union_criteria);
        for($i = 1; $i <= $sql_keyword_union_criteria_count; $i++)
            {
        ...
        $sql_keyword_union_aggregation_count = count($sql_keyword_union_aggregation);
        for($i = 1; $i <= $sql_keyword_union_aggregation_count; $i++)
            {
        ...
        $sql_keyword_union_or_count = count($sql_keyword_union_or);
        if($sql_keyword_union_or_count != $sql_keyword_union_criteria_count)
            {
        ...
        for($i = 0; $i < $sql_keyword_union_or_count; $i++)
            {
            if($i == 0)
                {
    ...
    // Standard Searches, we haveereached this far without returning, this must be a standard (non-special) search.
    ...
    // In case score has not been set (i.e. empty search).
    if($score == "")
        {
    ...
    // Execute query but only ask for ref columns back from mysql_query(); We force verbatim query mode on (and
    // restore it afterwards) as there is no point trying to strip slashes etc. just for a ref column.
    if($return_refs_only)
        {
    ...
    // All keywords resolved OK, but there were no matches.  Remove keywords, least used first, until we get results.
    $lsql = "";
    $omitmatch = false;
    $keywords_count = count($keywords);
    for($n = 0; $n < $keywords_count; $n++)
...
// Take the current search URL and extract any nodes (putting into buckets) removing terms from $search.
// UNDER DEVELOPMENT: Currently supports: @@!<node id> (NOT) and @@<node id>@@<node id> (OR).
function resolve_given_nodes(&$search, &$node_bucket, &$node_bucket_not)
    ...
    foreach($words[0] as $word)
        {
        // Remove the entire word from the search string.
        $search = str_replace($word, '', $search);

        preg_match_all('/' . NODE_TOKEN_PREFIX . '(' . NODE_TOKEN_NOT . '*)(\d+)/', $word, $tokens);

        // You are currently only allowed NOT condition for a single token within a single word.
        if(count($tokens[1]) == 1 && $tokens[1][0] == NODE_TOKEN_NOT)
            {
            // Add the node number to the node_bucket_not.
            $node_bucket_not[] = $tokens[2][0];
            continue;
            }
