...
function comments_submit()
    ...
    if ($comment_flag_ref != 0)
        ...
        // Add comment anchor to end of URL.
        $comment_flag_url .= "#comment${comment_flag_ref}";
        ...
        // Set so that the subsequent getval() function will pick up this comment flagged in the show comments function (headers have already been sent before cookie set).
        $_POST["comment${comment_flag_ref}flagged"] = "true";
    ...
    // We do not want to insert an empty comment or an orphan.
    if((getvalescaped("body","") == "") ||
    ...
    // Just in case not caught in submit form.
    if (strlen ($body) > $comments_max_characters) $body = substr ($body, 0, $comments_max_characters);
...
function comments_show($ref, $bcollection_mode = false, $bRecursive = true, $level = 1)
    ...
    // Pass this JavaScript function the "this" from the submit button in a form to post it via AJAX call, then refresh the "comments_container".
    if ($level == 1)
        {
        echo<<<EOT
...
EOT;
    ...
    else // Look for child comments, regardless of what type of comment.
        {
