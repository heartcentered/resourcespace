 ...
 *
 * Parses the plugins directory to look for a pluginname.yaml file and adds the plugin to the plugins database, setting
...
function activate_plugin($name)
    ...
    foreach(array_keys($plugin_yaml) as $thekey)
        ...
        // Add/Update plugin information, check if the plugin is already in the table.
 ...
 *
 * Blanks the inst_version field in the plugins database, which has the effect of deactivating the plugin while
 * maintaining any configuration that is stored in the database.
 *
 ...
 *
 * Replaces config value in plugins table with NULL.  Note, this function will operate on an activated plugin as well
 * so its configuration can be 'defaulted' by the plugin's configuration page.
 *
...
/**
 * Load plugin .yaml file.  Load a YAML file for a plugin and return an array of its values.
 *
...
function get_plugin_yaml($path, $validate = true)
    {
    // We are not using a full YAML structure, so this parsing function will do. If validate is false, this function
    // will return an array of blank values if a YAML file is not available.
...
/**
 * A subset json_encode function that only works on $config arrays but has none of the version-to-version variability
 * and other "unusual" behavior of PHP's implementation.
 *
 * @param $config mixed a configuration variables array. This *must* be an array whose elements are UTF-8 encoded
 * strings, booleans, numbers or arrays of such elements and whose keys are either numbers or UTF-8 encoded strings.
 * @return json encoded version of $config or null if $config is beyond our capabilities to encode.
 */
function config_json_encode($config)
    ...
    foreach($config as $name => $value)
        ...
        else // Give up; beyond our capabilities.
            {
            return NULL;
            }
        $output .= ', ';
...
/**
 * Utility function to encode the passed string to something that conforms to the JSON spececification for a string.
 * JSON does not allow strings with double-quotes, backslashes or control characters in them. For double-quote and
 * backslash, the encoding is '\"' and '\\' respectively.  The encoding for control characters is of the form '\uxxx'
 * where "xxx" is the UTF-8 4-digit hex value of the encoded character.
 *
...
/**
 * Return plugin config stored in plugins table for a given plugin name.  Queries the plugins table for a stored
 * config value and, if found, unserializes the data and returns the result.  If config is not found, returns null.
 *
...
/**
 * Store a plugin's configuration in the database.  Serializes the $config parameter and stores in the config
...
function set_plugin_config($plugin_name, $config)
    ...
    // We record the activity before running the query because log_activity() is trying to be clever and figure out
    // the old value which will make the new value also show up (incorrectly) as the old value.
...
/**
 * Check is a plugin is activated.  Returns TRUE is a plugin is activated in the plugins database.
 *
...
/**
 * Handle the POST for an upload of a plugin configuration (.rsc) file.  Typically invoked near the beginning of a
 * plugin's setup.php file something like this:
 *
 *  if (getval('upload','')!='')
 *      {
...
/**
 * Display HTML form for uploading a plugin configuration (.rsc) file.  Typically, invoked in a plugins's setup.php
 * file just after the form for setting individual plugin configuration parameters.
 *
...
function display_rsc_upload($upload_status)
    ...
    // i.e. if before json support in PHP.
...
/**
 * Generate the first half of the "guts" of a plugin setup page from a page definition array. This function deals
 * with processing the POST that comes (usually) as a result of clicking on the Save Configuration button.
 *
 * The page definition array is typically constructed by a series of calls to config_add_xxxx functions (see below).
 * See the setup page for the sample plugin for information on how to use this and the associated functions.
 *
 * If wishing to store array of values in one config option, in your setup page have something like the following
 * which adds a single definition for each key of your config option:
 ...
 *   }
 * The key can consist of numbers, letters or an underscore contained within "" or ''. If using numbers you do not need the quote marks
 *
 *
...
/**
 * Generate the second half of the "guts" of a plugin setup page from a page definition array. The page definition
 * array is typically constructed by a series of calls to config_add_xxxx functions (see below). See the setup page
 * for the sample plugin for information on how to use this and the associated functions.
 *
...
function config_gen_setup_html($page_def, $plugin_name, $upload_status, $plugin_page_heading, $plugin_page_frontm = '')
    ...
    foreach ($page_def as $def)
        ...
        // This comes first so overriding the below is possible.
        hook("custom_config_def", '', array($def));
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a comma-separated
 * list text entry configuration variable to the setup page.
 *
 ...
 * @param boolean $password     Whether this is a "normal" text-entry field or a password-style field. Defaulted to
 *                               false.
 ...
 * @param string array $choices The array of choices, the options in the select block. The keys are used as the
 *                               values of the options, and the values are the choices the user sees. (But see
 *                               $usekeys, below.) Usually a $lang entry whose value is an array of strings.
 * @param boolean $usekeys  Tells whether to use the keys from $choices as the values of the options. If set to false
 *                           the values from $choices will be used for both the values of the options and the text
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a multi select
 ...
 *                               to FALSE, the values from $choices will be used for both the values of the options
...
/**
 * Generate an HTML single-select block for selecting one of the RS users.  The user key (i.e., the value from the
 * "ref" column of the user table) of the selected user is the value posted.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a single RS user
 * select configuration variable to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
 * @param integer $width        The width of the input field in pixels. Default: 300.
...
/**
 * Generate an HTML multi-select block for selecting from among RS users.  An array consisting of the user keys
 * (i.e., values from the "ref" column of the user table) for the selected users is the value posted.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a multiple RS user
 * select configuration variable to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a single RS group
 * select configuration variable to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
...
/**
 * Generate an HTML multi-select block for selecting from among RS user groups.  An array consisting of the group
 * keys (i.e., values from the "ref" column of the usergroup table) for the selected groups is the value posted.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 * @param integer array $current The current value of the config variable being set.
...
/**
 * Generate an HTML multi-select + options block for selecting multiple the RS field types. The selected field type
 * is posted as an array of the values of the "ref" column of the selected field types.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 * @param integer array $current The current value of the config variable being set
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a multiple RS
 * field-type select configuration variable to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
 * @param integer $width        The width of the input field in pixels. Default: 300.
...
/**
 * Generate an HTML single-select + options block for selecting one of the RS resource types. The selected field type
 * is posted as the value of the "ref" column of the selected resource type.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 * @param integer $current  The current value of the config variable being set
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a single RS
 * resource-type select configuration variable to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
 * @param integer $width        The width of the input field in pixels. Default: 300.
...
/**
 * Generate an HTML multi-select check boxes block for selecting multiple the RS resource types. The selected field
 * type is posted as an array of the values of the "ref" column of the selected resource types.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 * @param integer array $current The current value of the config variable being set
...
/**
 * Generate an HTML single-select + options block for selecting from among rows returned by a database query in which
 * one of the columns is the unique key (by default, the "ref" column) and one of the others is the text to display
 * (by default the "name" column). The value posted is the value at the intersection of the selected rown with the
 * column given by the $ixcol variable.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 ...
 * @param string $ixcol     The key in $choices (i.e., the db column) for the value of the choice. Defaulted to 'ref'.
 * @param string $dispcolA  The key in $choices (i.e., the db column) for the text to display to the user. Defaulted to 'name'.
 * @param string $dispcolB  The key in $choices (i.e., the db column) for secondary text to display to the user.
 *                           Defaulted to '' indicating that only $dispcolA is to be displayed.
 ...
 * configuration variable whose value is chosen from among the results of a db query to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
 * @param array $choices        The array of db rows that make up the choices.
...
/**
 * Generate an HTML multi-select + options block for selecting from among rows returned by a database query in which
 * one of the columns is the unique key (by default, the "ref" column) and one of the others is the text to display
 * (by default the "name" column). The value posted is an array of the values of the column given by the $ixcol
 * variable for the rows selected.
 *
 * @param string $name      The name of the select block. Usually the name of the config variable being set.
 * @param string $label     The user text displayed to label the select block. Usually a $lang string.
 * @param string array $current The current value of the config variable being set.
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a multi select
 * configuration variable whose values are chosen from among the results of a db query to the setup page.
 *
 * @param string $config_var    The name of the configuration variable to be added.
 * @param string $label         The user text displayed to label the select block. Usually a $lang string.
 * @param array $choices        The array of db rows that make up the choices.
...
/**
 * Return a data structure that will instruct the configuration page generator functions to add a hidden
...
// Deprecated: Use config_text_input instead.                          <------------Is this function still needed?
...
// Deprecated: Use config_multi_user_select instead.                   <------------Is this function still needed?
...
// Deprecated: Use config_boolean_select instead.                      <------------Is this function still needed?
...
// Deprecated: Use config_single_select instead.                       <------------Is this function still needed?
