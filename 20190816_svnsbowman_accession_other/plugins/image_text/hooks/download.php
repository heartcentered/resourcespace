...
function HookImage_textDownloadModifydownloadfile()
	...
	# Locate imagemagick.
		...
		if ($path != $tmpfile  && $image_text_save_file) # If this is not a temporary file having metadata written, then copy it to the filestore for future use.
			{
...
function HookImage_textDownloadBeforedownloadresourceexit()
+    {
+    global $path, $image_text_save_file;
+
+    // Delete image text temp file.
+    if (!$image_text_save_file && is_file($path) && strpos($path, "_image_text_result") !== false)
+        {
+        unlink($path);
+        }
+    }
