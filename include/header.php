...
if (isset($js_cidr))
    {
    include_once ("cidr_match/CIDRmatch.php");
...
<?php
// Option to use Content Delivery Network (CDN)-hosted jQuery and jQueryUI files to reduce server load and bandwidth (~343 kB reduction).
if ($js_cdn && !$cidrMatch)
    { ?>
...
else // Use local jQuery and jQueryUI files.
    { ?>
...
<?php if ($pagename=="login")
    {
    ?><script type="text/javascript" src="<?php echo $baseurl?>/lib/js/jquery.capslockstate.js"></script><?php
    } ?>
<!--[if lte IE 9]><script src="<?php echo $baseurl?>/lib/historyapi/history.min.js"></script><![endif]-->

<?php if ($image_preview_zoom && $js_cdn && !$cidrMatch) // Use CDN-hosted file.
    {
    ?><script src="https://unpkg.com/jquery-zoom@1.7.18/jquery.zoom.js" integrity="sha384-11brNMEI2V9q0Zslh3fDAxByHgU62MicMBPCyxIA6iPEFDPkf37AUdkWMHAnuS1N" crossorigin="anonymous"></script><?php
    }
elseif ($image_preview_zoom) // Use local file.
    {
    ?><script src="<?php echo $baseurl?>/lib/js/jquery.zoom.js"></script><?php
    } ?>
...
<?php
    ...
    <script type="text/javascript" src="<?php echo $baseurl_short;?>lib/plupload_2.1.8/jquery.plupload.queue/jquery.plupload.queue.min.js?<?php echo $css_reload_key;?>"></script> <?php
    } ?>
<?php
if($videojs && ($keyboard_navigation_video_search || $keyboard_navigation_video_view || $keyboard_navigation_video_preview))
    { ?>
    <script type="text/javascript" src="<?php echo $baseurl_short?>lib/js/videojs-extras.js?<?php echo $css_reload_key?>"></script> <?php
    }
...
<script type="text/javascript">
    ...
    if($chosen_dropdowns_collection)
        { ?>
        var chosenCollectionThreshold='<?php echo $chosen_dropdowns_threshold_collection ?>'; <?php
        }
...
<?php if ($pagename!="preview_all")
    {
    ?><!--[if lte IE 7]> <link href="<?php echo $baseurl?>/css/globalIE.css?css_reload_key=<?php echo $css_reload_key?>" rel="stylesheet" type="text/css"  media="screen,projection,print" /> <![endif]--><?php
    } ?>
...
if(!hook("customloadinggraphic"))
    { ?>
    <div id="LoadingBox"><i aria-hidden="true" class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i></div> <?php
    }
...
if (($pagename!="preview" || $preview_header_footer) && $pagename!="preview_all")
    ...
    if ($use_theme_as_home)
        {
        $homepage_url=$baseurl."/pages/themes.php";
        }
...
if(!hook('replace_header_text_logo'))
    ...
    if($header_text_title)
        ...
        <?php if ($applicationdesc!="")
                { ?>
                <div id="TextDesc"><?php echo i18n_get_translated($applicationdesc);?></div> <?php
                }
        }
    else
        {
        ...
        if($header_link && ($k=="" || $internal_share_access))
            { ?>
            <a href="<?php echo $linkUrl; ?>" onClick="return CentralSpaceLoad(this,true);" class="HeaderImgLink"><img src="<?php echo $header_img_src; ?>" id="HeaderImg" ></img></a> <?php
            }
        else
            { ?>
            <div class="HeaderImgLink"><img src="<?php echo $header_img_src; ?>" id="HeaderImg"></img></div> <?php
            }
        }
    }
...
if (!$header_search)
    ...
    if ($modified_omit_searchbar_pages)
        {
        $omit_searchbar_pages = $modified_omit_searchbar_pages;
        }
