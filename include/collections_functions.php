...
function collection_download_use_original_filenames_when_downloading(&$filename, $ref, $collection_download_tar, &$filenames)
    ...
    global $pextension, $usesize, $subbed_original, $prefix_resource_id_to_filename, $prefix_filename_string, $server_charset,
           $download_filename_id_only, $deletion_array, $use_zip_extension, $copy, $exiftool_write_option, $p, $size, $aws_s3;
    ...
    if(!($collection_download_tar || $use_zip_extension))
        ...
        if(!$copy && $exiftool_write_option)
            ...
            if($aws_s3)
                ...
                // Strip $storagedir and trailing slash from path if present for AWS S3 bucket location.
                ...
                if($path_check == DIRECTORY_SEPARATOR)
                    {
                    $path = substr($p,1); // Strip leading slash in path.
                    }
                debug("COLLECTION_FUNCTIONS/ColDLOrgFilename S3 path: " . $p);

                // Get metadata on AWS S3 file and determine if file exists from metadata or lack thereof.
                try {
                    ...
                    {
                    debug("COLLECTION_FUNCTIONS/ColDLOrgFilename S3 Check Metadata Error: " . $e->getMessage());
                    return false;
                    }

                // Download file from AWS S3 bucket to $newpath.
                try {
                    $aws_s3_getobject = true;
                    ...
                    {
                    debug("COLLECTION_FUNCTIONS/ColDLOrgFilename S3 Download Error: " . $e->getMessage());
                    }

                // Check for successful file download from AWS S3 bucket to $newpath.
                if(!file_exists($newpath) || !is_readable($newpath))
                    {
                    debug("COLLECTION_FUNCTIONS/ColDLOrgFilename S3 Download Failure");
                    return false;
                    }
