...
function createTempFile($path, $uniqid, $filename, $aws_s3_download=false)
    ...
    // If using AWS S3 storage, get original file from S3 bucket instead.
    if($aws_s3 && $aws_s3_download)
        {
        include_once __DIR__ . '/aws_sdk.php';
        global $aws_bucket;

        // Check AWS S3 file path for a leading slash and if present, delete it.
        $path_check = substr($path, 0, 1);
        if($path_check == DIRECTORY_SEPARATOR)
            {
            $path = substr($path,1); // Strip leading slash in path.
            }
        debug("RESOURCE_FUNCTIONS/CREATETEMPFILE S3 Download Path: " . $path);

        // Check if file exists in specified AWS S3 bucket.
        try {
            $s3result = $s3Client->doesObjectExist($aws_bucket, $path);
            if($s3result == 1)
                {
                $s3result = "Ok";
                }
            debug("RESOURCE_FUNCTIONS/CREATETEMPFILE doesObjectExist Check: " . $s3result);
            }
        catch (Aws\S3\Exception\S3Exception $e) // AWS S3 bucket file exists failure.
            {
            debug("RESOURCE_FUNCTIONS/CREATETEMPFILE doesObjectExist Error: " . $e->getMessage());
            return false;
            }
        }
    else // Normal filestore file check.
        {
    ...
    debug("RESOURCE_FUNCTIONS/CREATETEMPFILE tmp Folder: " . $tmp_dir);
    ...
    if($aws_s3 && $aws_s3_download)
        ...
        try {
            ...
            {
            debug("RESOURCE_FUNCTIONS/CREATETEMPFILE S3 Download Error: " . $e->getMessage());
            }

        // Check for successful file download from AWS S3 bucket to the tmp location.
        if(file_exists($tmpfile) && is_readable($tmpfile))
            {
            debug("RESOURCE_FUNCTIONS/CREATETEMPFILE S3 File Downloaded To " . $tmpfile);
            }
        else // AWS S3 download failure.
            {
            debug("RESOURCE_FUNCTIONS/CREATETEMPFILE S3 File Download Failure for " . $path);
            return false;
            }
        }
...
function write_metadata($path, $ref, $uniqid="", $aws_s3_download=false)
    ...
    global $exiftool_remove_existing, $storagedir, $exiftool_write, $exiftool_write_option, $exiftool_no_process, $mysql_charset, $exiftool_write_omit_utf8_conversion, $aws_s3;
    ...
    if(false != $exiftool_fullpath && $exiftool_write && $exiftool_write_option && !in_array($extension, $exiftool_no_process))
        ...
        if($aws_s3 && $aws_s3_download)
            ...
            if($path_check == DIRECTORY_SEPARATOR)
                {
                $path = substr($path,1); // Strip leading slash in path for AWS S3.
                }
            debug("RESOURCE_FUNCTIONS/WRITE_METADATA S3 path: " . $path);
            ...
            try {
                ...
                debug("RESOURCE_FUNCTIONS/WRITE_METADATA doesObjectExist: " . $s3result);
                }
            catch (Aws\S3\Exception\S3Exception $e)
                {
                debug("RESOURCE_FUNCTIONS/WRITE_METADATA doesObjectExist Error: " . $e->getMessage());
                }

            // Determine tmp directory.
            $tmp_dir = get_temp_dir(false, $uniqid);
            debug("RESOURCE_FUNCTIONS/WRITE_METADATA tmp folder: " . $tmp_dir);
            ...
            try {
                ...
                {
                debug("RESOURCE_FUNCTIONS/WRITE_METADATA S3 Download Error: " . $e->getMessage());
                }

            // Check for successful file download from AWS S3 bucket to the tmp location.
            if(file_exists($tmpfile) && is_readable($tmpfile))
                {
                debug("RESOURCE_FUNCTIONS/WRITE_METADATA S3 File Downloaded To: " . $tmpfile);
                }
            else // S3 file download failure.
                {
                debug("RESOURCE_FUNCTIONS/WRITE_METADATA S3 Download Failure for " . $path);
                return false;
                }
            ...
            {
            debug("RESOURCE_FUNCTIONS/WRITE_METADATA createTempFile Path: " . $path);
